// tests/unit/bot/features/ticket/commands/usecases/ticketSettingsSetup.test.ts
...
describe("bot/features/ticket/commands/usecases/ticketSettingsSetup", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("既にカテゴリに設定がありパネルメッセージが存在する場合はエラー応答", async () => {
    const { handleTicketSettingsSetup } = await import(
      "@/bot/features/ticket/commands/usecases/ticketSettingsSetup"
    );
    ...
    await handleTicketSettingsSetup(interaction as never, "guild-1");
