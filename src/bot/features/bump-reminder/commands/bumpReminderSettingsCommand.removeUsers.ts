// src/bot/features/bump-reminder/commands/bumpReminderSettingsCommand.removeUsers.ts
// bump-reminder-settings remove-mention-users 実行処理
 ...
 */
export async function handleBumpReminderSettingsRemoveUsers(
  ...
  // 登録ユーザーを取得
  const config =
    await getBotBumpReminderSettingsService().getBumpReminderSettingsOrDefault(
      guildId,
    );
  ...
  collector.on("collect", async (i) => {
    ...
    if (i.customId === CUSTOM_ID.DELETE) {
      ...
      const currentSettings =
        await settingsService.getBumpReminderSettingsOrDefault(guildId);
      currentSettings.mentionUserIds = currentSettings.mentionUserIds.filter(
        (id) => !selectedIds.has(id),
      );
      await settingsService.saveBumpReminderSettings(guildId, currentSettings);
