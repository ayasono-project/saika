// tests/unit/bot/features/reaction-role/commands/usecases/reactionRoleSettingsTeardown.test.ts
...
describe("bot/features/reaction-role/commands/usecases/reactionRoleSettingsTeardown", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("パネルが0件の場合はエラー応答を返す", async () => {
    const { handleReactionRoleSettingsTeardown } = await import(
    ...
    await handleReactionRoleSettingsTeardown(interaction as never, "guild-1");
