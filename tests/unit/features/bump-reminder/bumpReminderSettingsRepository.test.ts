...
describe("shared/database/repositories/bumpReminderSettingsRepository", () => {
  async function loadModule() {
    return import("@/features/bump-reminder/bumpReminderSettingsRepository");
  }
  ...
  describe("setBumpReminderEnabled", () => {
    it("channelId が指定された場合に enabled=true で upsert すること", async () => {
      ...
      expect(prisma.guildBumpReminderSettings.upsert).toHaveBeenCalledWith(
    ...
    it("channelId が未指定の場合は update に channelId を含めないこと", async () => {
      ...
      const call = prisma.guildBumpReminderSettings.upsert.mock.calls[0][0];
  ...
  describe("updateBumpReminderSettings", () => {
    it("mentionUserIds を JSON 文字列化して upsert すること", async () => {
      ...
      const { BumpReminderSettingsRepository } = await loadModule();
      const repo = new BumpReminderSettingsRepository(prisma as never);
      await repo.updateBumpReminderSettings("guild-1", {
        enabled: true,
        channelId: "ch-1",
        mentionRoleId: "role-1",
