// tests/unit/bot/features/vac/commands/usecases/vacSettingsCreateTrigger.test.ts
...
} from "@/bot/features/vac/commands/helpers/vacSettingsTargetResolver";
...
vi.mock(
  "@/bot/features/vac/commands/helpers/vacSettingsTargetResolver",
  () => ({
...
describe("bot/features/vac/commands/usecases/vacSettingsCreateTrigger", () => {
  ...
  it("ギルドコンテキストが存在しない場合にValidationErrorをスローする", async () => {
    ...
    await expect(
      handleVacSettingsCreateTrigger(interaction as never, "guild-1"),
  ...
  it("トリガーチャンネルを作成してDBに保存し、エフェメラルで成功応答する", async () => {
    ...
    await handleVacSettingsCreateTrigger(interaction as never, "guild-1");
