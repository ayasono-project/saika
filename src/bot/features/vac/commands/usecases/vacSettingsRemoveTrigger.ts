...
const CUSTOM_ID = {
  SELECT: "vac-settings:trigger-remove-select",
  CONFIRM: "vac-settings:trigger-remove-confirm",
} as const;

/**
 * vac-settings remove-trigger-vc を実行する
 * 全トリガーチャンネルをセレクトメニューで表示し、複数選択→削除する
 * @param interaction コマンド実行インタラクション
 * @param guildId 実行対象ギルドID
 */
export async function handleVacSettingsRemoveTrigger(
  ...
  const service = getBotVacSettingsService();
  const config = await service.getVacSettingsOrDefault(guildId);
