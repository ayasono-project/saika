...
export async function handleReset(
  ...
  const confirmEmbed = createWarningEmbed(
    tInteraction(locale, "guildSettings:embed.description.reset_confirm"),
    {
      title: tInteraction(locale, "guildSettings:embed.title.reset_confirm"),
    },
  );

  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder()
      .setCustomId(GUILD_SETTINGS_CUSTOM_ID.RESET_CONFIRM)
      .setEmoji("🗑️")
      .setLabel(tInteraction(locale, "common:ui.button.reset_confirm"))
      .setStyle(ButtonStyle.Danger),
    new ButtonBuilder()
      .setCustomId(GUILD_SETTINGS_CUSTOM_ID.RESET_CANCEL)
      .setEmoji("❌")
  ...
  collector.on("collect", async (i) => {
    if (i.customId === GUILD_SETTINGS_CUSTOM_ID.RESET_CONFIRM) {
      // リセット実行
      ...
      const successEmbed = createSuccessEmbed(
        tInteraction(locale, "guildSettings:user-response.reset_success"),
      );
    ...
    } else if (i.customId === GUILD_SETTINGS_CUSTOM_ID.RESET_CANCEL) {
      const cancelEmbed = createSuccessEmbed(
        tInteraction(locale, "guildSettings:user-response.reset_cancelled"),
      );
