...
| `/vc-recruit-settings setup` | 指定カテゴリーに募集作成・募集一覧チャンネルをセットで作成 |
| `/vc-recruit-settings teardown` | 設定済みカテゴリーを選択して募集チャンネルセットを削除 |
| `/vc-recruit-settings add-role` | メンション候補に使えるロールを追加 |
| `/vc-recruit-settings remove-role` | メンション候補からロールを削除 |
| `/vc-recruit-settings view` | 現在の設定を確認 |
...
5. 作成したVCのIDを DB に保存（createdVoiceChannelIds に追加）
   ↓
6. 設定されたメンション（ある場合）を付与して 募集一覧 に募集メッセージを送信
   （VCリンクは作成した新規VCのリンク。メッセージには「🎤 VCに参加」「✏️ VC名を変更」「🔇 募集終了」「🗑️ 募集を削除」ボタンを添付）
   ↓
7. 募集メッセージにスレッドを自動作成（スレッド名: `{募集者表示名}の募集` / アーカイブ時間: 設定値）
   ↓
8. エフェメラルメッセージを成功メッセージ＋投稿リンクに更新
   「✅ 募集を投稿しました → 投稿を確認する」
...
- 同一カテゴリーに VAC トリガーチャンネルが存在する場合は、そのチャンネルをVC選択肢から除外（VAC のトリガーチャンネルIDは `VacSettings.triggerChannelIds` から取得）
- ギルドの AFK チャンネル（`Guild.afkChannelId`）が候補カテゴリー内にある場合も VC 選択肢から除外（VAC トリガー除外と同じ扱い）
- 実行ユーザーが候補VC（同一カテゴリ内、AFK／VAC トリガー以外）に参加中の場合、そのVCをセレクトメニューの default 選択として表示する。未参加もしくは入室中VCが候補外のときは「🆕 新規VC作成」を default 選択とする
- メンションを複数選択した場合、選択したすべてのロールがメッセージ本文の先頭にメンションとして付与される
- 既存VC選択時・新規VC作成時ともに投稿メッセージの表示形式は同一
- 新規作成VCの設定変更（名前・人数制限）・切断・移動・AFK移動は `/vc`・`/afk` コマンドで行う（[VC_COMMAND_SPEC.md](VC_COMMAND_SPEC.md) / [AFK_SPEC.md](AFK_SPEC.md)）
- 新規作成VCの名前は募集作成時（ステップ2モーダルの「新規VC名」フィールド）で事前設定できるほか、募集メッセージの「✏️ VC名を変更」ボタン、または `/vc rename`（参加中のメンバー）から後から変更できる
...
作成された新規VCの設定変更（名前・人数制限）・メンバーの切断/移動/AFK移動は、[`/vc`・`/afk` コマンド](VC_COMMAND_SPEC.md)で行う（操作パネルは 2026-05-30 に廃止）。
...
## /vc-recruit-settings setup

### コマンド定義

**コマンド**: `/vc-recruit-settings setup`
...
/vc-recruit-settings setup category:TOP
/vc-recruit-settings setup category:ゲームカテゴリー
/vc-recruit-settings setup
...
## /vc-recruit-settings teardown

### コマンド定義

**コマンド**: `/vc-recruit-settings teardown`
...
```mermaid
flowchart TD
    A["/vc-recruit-settings teardown"] --> B{設定済みセットアップが存在するか？}
...
## /vc-recruit-settings add-role

### コマンド定義

**コマンド**: `/vc-recruit-settings add-role`
...
## /vc-recruit-settings remove-role

### コマンド定義

**コマンド**: `/vc-recruit-settings remove-role`
...
## /vc-recruit-settings view

### コマンド定義

**コマンド**: `/vc-recruit-settings view`
...
- リセットコマンドは不要（`/vc-recruit-settings teardown` で全選択により一括撤去可能）
...
| キー | 用途 | ja | en |
| --- | --- | --- | --- |
| `vc-recruit-settings.description` | コマンド説明 | VC募集機能の設定（サーバー管理権限が必要） | VC recruit feature settings (requires Manage Server) |
| `vc-recruit-settings.setup.description` | サブコマンド説明（setup） | VC募集チャンネルをセットアップ | Set up VC recruit channels |
| `vc-recruit-settings.setup.category.description` | オプション説明（category） | 作成先カテゴリー（TOP またはカテゴリー名。未指定時は実行チャンネルのカテゴリー） | Target category (TOP or category name; defaults to this channel's category) |
| `vc-recruit-settings.setup.category.top` | カテゴリー選択肢TOP | TOP（カテゴリーなし） | TOP (No category) |
| `vc-recruit-settings.setup.thread-archive.description` | オプション説明（thread-archive） | 招募スレッドの自動アーカイブ時間（1h/24h/3d/1w、未指定: 24h） | Auto-archive duration for recruit threads (1h/24h/3d/1w; default: 24h) |
| `vc-recruit-settings.teardown.description` | サブコマンド説明（teardown） | VC募集チャンネルを削除（選択UI経由） | Remove VC recruit channels (via selection UI) |
| `vc-recruit-settings.add-role.description` | サブコマンド説明（add-role） | メンション候補ロールを追加 | Add a role to the mention candidates |
| `vc-recruit-settings.remove-role.description` | サブコマンド説明（remove-role） | メンション候補ロールを削除 | Remove a role from the mention candidates |
| `vc-recruit-settings.view.description` | サブコマンド説明（view） | 現在のVC募集設定を表示 | Show current VC recruit settings |
...
| 依存先 | 内容 |
| --- | --- |
| VAC (VC自動作成) | vc-recruit は `voiceStateUpdate` による空VC自動削除を行わないため、VAC の自動削除処理とは干渉しない。VAC トリガーチャンネルはVC選択肢から除外（`VacSettings.triggerChannelIds` から取得） |
