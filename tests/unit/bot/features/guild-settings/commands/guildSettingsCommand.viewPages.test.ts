...
vi.mock("@/bot/services/botCompositionRoot", () => ({
  getBotGuildSettingsService: () => ({ getSettings: getConfigMock }),
}));
...
import { buildGuildSettingsPage } from "@/bot/features/guild-settings/commands/guildSettingsCommand.viewPages";

// ギルド設定ページの Embed 生成ロジックを検証
describe("bot/features/guild-settings/commands/guildSettingsCommand.viewPages", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getConfigMock.mockResolvedValue(null);
  });

  describe("buildGuildSettingsPage - ギルド設定", () => {
    it("設定が存在する場合は locale と errorChannelId が表示されること", async () => {
      ...
      const embed = (await buildGuildSettingsPage(
        "g1",
        "ja",
      )) as unknown as Record<string, unknown>;
      expect(embed.title).toBe("guildSettings:embed.title.view");
