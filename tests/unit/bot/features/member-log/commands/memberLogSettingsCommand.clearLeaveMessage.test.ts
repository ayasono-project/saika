// tests/unit/bot/features/member-log/commands/memberLogSettingsCommand.clearLeaveMessage.test.ts

import { ValidationError } from "@ayasono/shared/core";
import { handleMemberLogSettingsClearLeaveMessage } from "@/bot/features/member-log/commands/memberLogSettingsCommand.clearLeaveMessage";
...
// handleMemberLogSettingsClearLeaveMessage の権限チェック・削除・応答フローを検証
describe("bot/features/member-log/commands/memberLogSettingsCommand.clearLeaveMessage", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("ガードが ValidationError を投げた場合にそれが伝播することを確認", async () => {
    ...
    await expect(
      handleMemberLogSettingsClearLeaveMessage(
  ...
  it("service.clearLeaveMessage が正しい guildId を引数に呼ばれることを確認", async () => {
    ...
    await handleMemberLogSettingsClearLeaveMessage(
      makeInteraction() as never,
      "guild-1",
    );
