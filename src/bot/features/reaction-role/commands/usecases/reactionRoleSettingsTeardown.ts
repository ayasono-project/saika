// src/bot/features/reaction-role/commands/usecases/reactionRoleSettingsTeardown.ts
...
/**
 * reaction-role-settings teardown サブコマンドを処理する
 * @param interaction コマンド実行インタラクション
 * @param guildId ギルドID
 */
export async function handleReactionRoleSettingsTeardown(
  interaction: ChatInputCommandInteraction,
  guildId: string,
): Promise<void> {
  const settingsService = getBotReactionRolePanelSettingsService();
  const allPanels = await settingsService.findAllByGuild(guildId);
...
async function filterExistingPanels(
  ...
  settingsService: { delete: (id: string) => Promise<void> },
