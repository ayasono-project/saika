// tests/unit/shared/features/vc-recruit/vcRecruitSettingsService.test.ts
// VcRecruitSettingsService のビジネスロジック・分岐・シングルトン挙動を検証する
...
} from "@/shared/features/vc-recruit/vcRecruitSettingsService";

describe("shared/features/vc-recruit/vcRecruitSettingsService", () => {
  const createRepositoryMock = () => ({
    getVcRecruitSettings: vi.fn<() => Promise<VcRecruitSettings | null>>(),
    updateVcRecruitSettings:
      vi.fn<(guildId: string, config: VcRecruitSettings) => Promise<void>>(),
  });
  ...
  const makeConfig = (
    overrides: Partial<VcRecruitSettings> = {},
  ): VcRecruitSettings => ({
    enabled: true,
    mentionRoleIds: [],
    setups: [],
  ...
  // ──────────────────────────────────────────────────────────
  // getVcRecruitSettingsOrDefault
  // ──────────────────────────────────────────────────────────

  it("設定が未登録の場合はデフォルト値を返し、呼び出しごとに別インスタンスになること", async () => {
    const repository = createRepositoryMock();
    repository.getVcRecruitSettings.mockResolvedValue(null);
    const service = new VcRecruitSettingsService(repository);

    const first = await service.getVcRecruitSettingsOrDefault("g1");
    const second = await service.getVcRecruitSettingsOrDefault("g1");

    expect(first).toEqual(DEFAULT_VC_RECRUIT_SETTINGS);
  ...
  it("設定が存在する場合は内容が一致した正規化コピーを返し、配列は元と別参照であること", async () => {
    ...
    repository.getVcRecruitSettings.mockResolvedValue(raw);
    const service = new VcRecruitSettingsService(repository);

    const config = await service.getVcRecruitSettingsOrDefault("g1");
  ...
  it("保存時に正規化済みコピーが repository へ渡され、入力オブジェクトとは別参照であること", async () => {
    const repository = createRepositoryMock();
    repository.getVcRecruitSettings.mockResolvedValue(null);
    repository.updateVcRecruitSettings.mockResolvedValue(undefined);
    const service = new VcRecruitSettingsService(repository);

    const input = makeConfig({ mentionRoleIds: ["r1"] });
    await service.saveVcRecruitSettings("g1", input);

    expect(repository.updateVcRecruitSettings).toHaveBeenCalledWith(
      "g1",
      expect.objectContaining({ mentionRoleIds: ["r1"] }),
    );
    // 渡されたオブジェクトが元と別参照であること（正規化コピー）
    const saved = repository.updateVcRecruitSettings.mock
      .calls[0][1] as VcRecruitSettings;
  ...
  it("新規セットアップを追加すると一覧に含まれ、機能が有効化されること", async () => {
    const repository = createRepositoryMock();
    repository.getVcRecruitSettings.mockResolvedValue(
  ...
  it("isCreatedVcRecruitChannel: 追跡対象外の VC の場合は false を返すこと", async () => {
    const repository = createRepositoryMock();
    repository.getVcRecruitSettings.mockResolvedValue(makeConfig());
    const service = new VcRecruitSettingsService(repository);
  ...
  // ──────────────────────────────────────────────────────────
  // createVcRecruitSettingsService ファクトリ関数
  // ──────────────────────────────────────────────────────────

  it("createVcRecruitSettingsService は VcRecruitSettingsService のインスタンスを返すこと", async () => {
    const { createVcRecruitSettingsService } = await import(
      "@/shared/features/vc-recruit/vcRecruitSettingsService"
    );
    const repository = createRepositoryMock();
    const service = createVcRecruitSettingsService(repository);
    expect(service).toBeInstanceOf(
      (await import("@/shared/features/vc-recruit/vcRecruitSettingsService"))
        .VcRecruitSettingsService,
    );
  });
});
