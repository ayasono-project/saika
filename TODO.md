...
| 3. Coolify移行(saika側) | デプロイ手段の刷新 | 完了 |
| **合計** | | **4+α** |
...
ayasono-webダッシュボードは saika の DB を直接参照せず、saika が公開する **Fastify REST API** を経由して設定の閲覧・編集を行う。saika 側にはこれまで Bot プロセスしか存在しないため、新たに Web 用 API レイヤを追加する。
...
- [ ] コマンド名を `-config` から `-settings` にリネーム（コマンド名・ファイル名・変数名・ドキュメント・DBテーブル名）
