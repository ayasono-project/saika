...
| 3.3.0 | export / import の削除・バグ修正（リリース前のレビューと実機確認で見つけた分を含む）・message-delete の全チャンネルにスレッドを含める・チケットを管理者権限でも操作できるようにした | minor（機能が減った） |
...
v3.3.0。リリース前の実機確認で、Bot をキックして入れ直した後に古いチケットを再オープンすると 50001 Missing Access で失敗したことから見つけた。Bot をサーバーから外すと、Discord はチャンネルに付けていた Bot 自身へのメンバー上書き（`createTicketChannel` が付ける閲覧・送信・埋め込みリンク・履歴）を消す。招待は最小権限（Administrator 無し）なので、v3.2.0 の遅延削除で設定が残ったまま30日以内に入れ直すと、それより前に作ったチケットのチャンネルに Bot が入れない。

- **操作が途中で失敗し、記録と画面が食い違っていた。** 再オープンは、記録を OPEN にして自動削除の予約を取り消した後で、通知の送信が 50001 で失敗していた。クローズ・再オープン・削除の前に Bot がそのチャンネルを扱えるかを確かめ（`ticketChannelAccess.ts`。閲覧できないチャンネルもキャッシュからは取れてしまうので、取得の成否ではなく `permissionsFor` で判定する）、扱えなければ押した本人に理由と付け直す手順を返して何も変えない。クローズ・再オープンは「通知を送る → 記録を更新（失敗したら送った通知を消す）→ 予約 → 送信の上書き → 前回の通知を消す」の順にした
- **自動削除は記録だけ消してチャンネルを残していた**（Bot から二度と操作できないチャンネルになる）。扱えないときは記録を消さずに保留し、1時間ごとに再試行する。単発のジョブは発火時に消えるので、再試行しないと権限を付け直しても次の再起動まで止まる。ログは最初の保留だけ warn にし、以降は予約のログも含めて debug にした
- **削除で「チャンネルの管理」を確かめていなかった**（以前から）。4権限だけ見て記録を先に消し、`channel.delete()` が失敗すると記録の無いチャンネルが残った。削除の経路（ボタン・`/ticket delete`・自動削除・撤去）では「チャンネルの管理」も確かめ、それだけが無いときは付け直しとは別の案内を返す。撤去は扱えないチケットを飛ばして warn を残す
- **入れ直したときに管理者へ知らせる。** 再導入（`guildCreate`）のときだけ、Bot が扱えないチケットのチャンネルと付け直す手順をエラー通知チャンネルへ1回送る。推奨どおり管理者専用にしたエラー通知チャンネルには同じ理由で Bot が送れないので、未設定か送れなかったときはオーナー宛の再導入 DM に件数と手順を載せる（そのためにチケットの同期を DM より前にした）。どちらの文面にも、ログなど Bot 用に権限を付けていた他の非公開チャンネルも付け直しが要ることを書いた。エラー通知チャンネルへの送信失敗のログは debug から warn に上げた（既定のログレベルでは失敗が何も残っていなかった）。Bot の停止中・切断中に入れ直した場合は、通知も DM も出ない（操作したときの案内は出る）
- マニュアルに、入れ直した後の動きと付け直す手順（カテゴリの権限を変えても反映されない）、残したいチケットは付け直したらすぐ再オープンすること（再試行の時刻は付け直した時刻と関係ないため）を書いた。あわせて、パネルはコマンドを実行したチャンネルに置かれ、チケットは指定したカテゴリの中に作られる、と書き直した（実機確認の途中で、パネルがカテゴリに置かれると読めたため）
- 開発環境の Bot はコマンドをテストサーバー専用で登録しているので、キックするとコマンドも消え、再起動するまで戻らない。本番は全サーバー共通の登録なので起きない

**実機で確かめたこと**（2026-09-26・テストサーバー）: エラー通知チャンネルが未設定のまま入れ直すと、オーナー DM に件数と手順の欄が出る。設定してから入れ直すと、エラー通知チャンネルに対象のチャンネルと手順が1回届き、DM には欄が出ない。古いチケットの再オープンは「Bot権限不足」の案内が出て、記録も予約も変わらない。チャンネルに Bot を追加して4権限を許可すると、再オープン・クローズ（自動削除の予約）が普通に通る。

### チケット：管理者権限でも削除・クローズ・再オープンできるようにし、権限不足の文面を具体的にした（2026-09-26 完了・本番デプロイ済み）

v3.3.0。実機確認で、サーバーオーナーがチケットの削除を押して「この操作を実行する権限がありません。」とだけ返ったことから。削除はスタッフロールだけ、クローズ・再オープンは作成者かスタッフロールで判定していたので、オーナーや管理者権限を持つメンバーでもスタッフロールが無いと削除できず、文面からは何が必要か分からなかった。

- 管理者権限（Administrator）を持つメンバーも、削除・クローズ・再オープンができるようにした（オーナーは Discord 上で全権限を持つ扱いなので含まれる）。作成者が削除できないのは今までどおり。判定は `canOperateTicket`（`ticketGuards.ts`）の1か所にまとめ、ボタンとコマンドの7経路すべてで使う（ロール ID の取り出しが3か所に分かれていた重複もなくなった）。押した人の権限を確かめてから Bot の権限を確かめる順序は変えていない
- 権限不足の文面を、削除用とクローズ・再オープン用に分け、「チケットを削除できるのは、スタッフロール（@…）を持つメンバーと、管理者権限を持つメンバーだけです。」のように、誰なら操作できるかを書いた。スタッフロールはメンションで出す。Web API からはスタッフロール0件の設定を作れるので、そのときは「未設定」と出す。タイトルはガイドラインの表に合わせて「権限不足」にした。使われなくなった `ticket:user-response.not_authorized` は消した

**実機で確かめたこと**（2026-09-26・テストサーバー）: スタッフロールを持たないオーナーが削除を押すと、Bot がチャンネルに入れないときは「Bot権限不足」の案内で止まり、入れるときは削除の確認が出て、削除まで通る。

### チケットの自動削除を25日以上にすると、クローズした直後に削除されていた（2026-09-26 完了・本番デプロイ済み）

v3.3.0。リリース前のレビュー（機能別5観点＋指摘ごとの反証）で見つけた、**以前から本番で起きていた不具合**。

- `jobScheduler.addOneTimeJob` が遅延をそのまま `setTimeout` に渡しており、Node は 2^31-1 ms（約24.8日）を超える値を **1ms に切り詰めて即時に発火する**（TimeoutOverflowWarning）。自動削除の日数はダッシュボードで1〜90日、コマンドでは上限なしで設定できるので、25日以上にしているとクローズした直後にチケットが会話ごと消えていた（クローズ通知には「N日後に自動削除」と出ていた）
- 上限（`MAX_TIMEOUT_DELAY_MS`）ずつ待ち、残りを同じジョブ ID で張り直すようにした。残りは時計ではなく各区間の長さを差し引いて求める（システム時刻を進めても早く発火しない）。`hasJob` / `removeJob` / `stopAll` / 同 ID の置き換えは張り直し中にも効く。NaN・Infinity の遅延は登録を拒否してエラーログを出す（そのまま渡すと 1ms で発火し、データを消すジョブが即座に走るため）
- 設定できる日数を24日までに狭める案は、ダッシュボードで選べる範囲が狭まるので採らなかった
- タイマーが長い待ちを扱えるようになると、「約24.8日を超えてクローズしていたチケットを再オープン」する経路が初めて通る。経過時間の列（`Ticket.elapsedDeleteMs`）は int4 なので、保存時（`TicketRepository.create` / `update`）に 0〜上限へ切り詰める。自動削除が少し遅くなるだけで、早く消えることは無い

### message-delete：スレッド対応で起きやすくなった失敗と、実行者の権限の抜け穴を直した（2026-09-26 完了・本番デプロイ済み）

v3.3.0。リリース前のレビューで見つけた。スレッドを対象にしたことで初めて通る、または起きやすくなった経路と、以前からの権限の抜け穴。

- **削除できないシステムメッセージで削除全体が止まっていた。** スレッドには開始メッセージ・名前変更・メンバー追加／除外のメッセージが入り、Discord の仕様で削除できない。1件だけのチャンクでは `bulkDelete` が単体の DELETE になって 50021 を投げ、以降のチャンネルが1件も消えなかった（今日メッセージから作ったスレッドがあるだけで `/message-delete days:1` が失敗する）。discord.js の `UndeletableMessageTypes` に入る種別をスキャン対象から外した（`Message#deletable` は Bot のキャッシュ上の権限も混ぜて判定し、理由の見えないまま収集から消えうるので使わない）
- **削除の失敗を局所化した。** チャンネルが使えないエラー（10003 / 50001 / 50013）はそのチャンネルの残りを打ち切り、それ以外の一括削除の失敗はそのチャンクだけ1件ずつに切り替える。スキャンも、チャンネルが使えないエラーのときはそのチャンネルだけ打ち切って続ける（それ以外の一時的な障害では、「対象が見つからない」と誤って表示しないよう、従来どおり収集の失敗として終える）
- **表示の文字数上限を超えて「削除に失敗」になっていた**（完了表示は38チャンネル以上、進捗表示は約63チャンネル以上）。どちらも先頭20行＋「ほか M チャンネル」にまとめ、文字数でも切る。削除後の表示や進捗表示の失敗で「削除に失敗」にしない。タイムアウトで途中まで消した場合も削除ログを出し、実際に消えた件数を表示する
- **実行者の権限を、実行したチャンネルでしか見ていなかった。** チャンネル未指定だと、実行者が見られないチャンネル（非公開スレッドを含む）までスキャンされ、プレビューで本文が見え、削除もできた。対象ごとに実行者が ViewChannel・ReadMessageHistory・ManageMessages を持つかを確かめ、非公開スレッドは ManageThreads を持つか参加していることを求める（参加は `force: true` で API に問い合わせる。キャッシュが古いと抜けた人を参加者と誤認するため）。GuildMember が取れないときはどのチャンネルも対象にしない（fail closed）。チャンネル未指定で除いたものは通知しない（見られないチャンネルの存在を知らせないため）。指定したチャンネルを実行者の権限不足で除いたときは、Bot の権限不足とは別の文言で通知する
- 初回フェッチの間も中断（「収集分を確認」・タイムアウト）が効くようにした

### チケット：設定が消えたカテゴリのチケットの扱いと、競合を直した（2026-09-26 完了・本番デプロイ済み）

v3.3.0。リリース前のレビューで見つけた、以前からの不具合。

- **パネル（設定）が消えたカテゴリのチケットは、クローズが何もせずに成功と返し、クローズ済みは再起動後に自動削除されなかった**（パネル設置チャンネルやパネルメッセージの削除、Web API の DELETE /tickets/:id の後）。「Bot からは操作せず自動削除も止める。理由と出口を本人に返し、同じカテゴリにパネルを作り直すと操作も自動削除も戻る」に統一した。設定が無いとスタッフロールも自動削除の日数も分からず、部分的に動かすと「スタッフが閉じられない」「日数不明で自動削除を決められない」など中途半端になるため。パネルの削除と同時にチケットも消す案は、パネル設置チャンネルを誤って消しただけで会話履歴が全部消えるので採らなかった。作り直したときは、残っている予約も作り直した設定の日数で組み直す。残り時間は closedAt から数えるので、止まっている間に期限を過ぎたチケットは作り直した時点で削除される（Bot の返信とダッシュボードの確認にもそう書いた）
- **自動削除が状態を見ずに消していた**（起動時・再導入時の復元の最中に再オープンされると、オープン中のチケットを消しうる）。発火時に記録と設定を読み直し、状態の確認と削除を1回のクエリ（`deleteIfClosed`）で行う
- **作成ボタンを続けて押すと上限を超えて作れた。** モーダルの送信時にも上限を確かめ直し、上限の確認から作成の完了までを (ギルド・カテゴリ・ユーザー) 単位のプロセス内の排他で囲む（Bot は1プロセスで動いているので足りる）。作成中に同じ人がもう一度送信したら「作成中です」と返して作らない
- 作成で記録の保存に失敗すると、チャンネルだけが残っていた。作ったチャンネルを消してからエラーを返す
- 再接続（再 IDENTIFY）で戻ったギルドでも、チケットを同期する（`guildAvailable`）。切断中に消されたチケットチャンネルが、次の再起動まで片付かなかったため。既に予約がある自動削除は組み直さない
- 自動削除が失敗したときのログを、成功時と別のキーにした。「予約した」ログは、タイマーが登録を受け付けたときだけ出す

### Bump：予約の取り消しでパネルも消し、ダッシュボードと全設定リセットでも取り消すようにした（2026-09-26 完了・本番デプロイ済み）

v3.3.0。リリース前のレビューで見つけた。

- **予約を取り消すと「<t:…:R>にリマインドが通知されます」のパネルが残り、次の Bump でも消えなかった**（次の Bump は pending の行しか見ないため）。今日の disable の修正で「disable → 予定時刻前に enable」でも残るようになっていた。取り消す前に pending 行からパネルの場所を控え、タイマーを止めてからパネルを消す（`cancelGuildBumpReminders`）。コマンドの disable・reset、ダッシュボードの無効化・リセット、全設定リセット（コマンド・Web API）で使う。Bot が退出したとき（`stopGuildJobsUsecase`）は Bot がいないので消せず、従来どおり予約だけ取り消す
- **ダッシュボードの無効化・リセットは予約を取り消していなかった**（コマンドだけ直っていた）
- **リセットの結果が経路で逆だった**（コマンドは機能が有効のまま、ダッシュボードは無効）。コマンド側の既定値（有効・全チャンネルで検知・メンションなし）に揃えた。一度も設定していないギルドの状態がこれで、ダッシュボードの確認文言も「初期値に戻します」だったため。リセットの確認画面の削除対象に「検知チャンネル」を足した
- 設定を先に保存してから予約を取り消す順に揃えた（取り消しと保存の間に検知した Bump が、古い設定を読んで新しい予約を作るのを防ぐ）。保存より前に設定を読み終えていた検知は、予約を登録した後に設定を読み直し、無効なら自分で予約を取り消してパネルを消す。発火時に機能が無効だった場合もパネルを消す
- 予約の管理マップで、実行中のタスクが同じキーの次の予約のエントリまで消していた競合を直した（自分の reminderId のときだけ消す）。取り消しログに「GuildId: 123:Disboard」と出ていたのを、ギルド ID とサービス名に分けた

### ギルド設定のリセットでエラー通知チャンネルが消えていなかった ほか（2026-09-26 完了・本番デプロイ済み）

v3.3.0。リリース前のレビューで見つけた、init からの不具合。`/guild-settings reset` と Web API の config reset が `errorChannelId: undefined` を渡しており、シリアライザーが undefined を「変更しない」として落とすため、エラー通知チャンネルが残っていた。「消す」は null で表すことにして直した（`GuildSettingsUpdate` 型）。テストは、コマンドと Web API の両経路でサービスからシリアライザーまでを実物にし、Prisma だけをモックする形にした（別々に見るテストでは、この組み合わせの不具合を検出できなかった）。

web（ダッシュボード）: VC自動募集のページに変数のボタン（{userMention} 等）が出ていなかったのを直し、チケットパネルを削除するときの確認に「残ったチケットはパネルを作り直すまで操作できない」旨を足した。開発用のモックも実際の API に合わせた。

### チケットの上限メッセージとログに値が埋め込まれていなかった（2026-09-26 完了・本番デプロイ済み）

v3.3.0。確認2のログで `{{userId}}` 等がそのまま出ていたのをきっかけに、全ロケールキーの `{{…}}` と呼び出し元が渡している値を src 全体で突き合わせた（キーを定数の表に置いている箇所は、表を使う側で値を渡しているかを個別に確認した）。

- **利用者に見えていたもの**: チケットの同時作成上限に達したときのメッセージに件数を渡しておらず、「チケットの同時作成上限（{{max}}件）に達しています。」と表示されていた
- **ログ**: チケットの作成（作成者・番号）・クローズ／再オープン／削除（操作した人。コマンドとボタンの両経路）が抜けていた。パネル設置の開始は、完了時と同じ「設置」のキーでチャンネル無しに出していたので `ticket:log.setup_started` を分けた。Bump リマインダーの保留中の取得失敗は `Id` を期待するキーに `guildId` を渡していたので専用のキーにした
- 導入時・再導入時の DM のログが同じ文言で区別できなかったので、再導入時用のキーを足した

### チケットのチャンネルを直接消すと、作成者が新しいチケットを作れなくなっていた（2026-09-26 完了・本番デプロイ済み）

v3.3.0。v3.3.0 の告知文の確認中に、「再導入でチケットの自動削除タイマーを組み直すと、外されていた間にチャンネルを整理されていたらエラーになるか」を調べて見つけた。

- **channelDelete はパネルを置いたチャンネルしか見ていなかった。** 管理者が Discord の画面からチケットチャンネルを直接消すと、DB の「オープン中のチケット」が残り続け、同時作成の上限（既定1件）に達したまま作成者が新しいチケットを作れなくなっていた。解消するにはチケット設定ごとリセットするしかなかった。**USER_MANUAL は「対応するチケットレコードが自動的にクリーンアップされます」と書いていた**が、実装が無かった。チケットチャンネルが消されたら、その記録と自動削除タイマーを消すようにした。Bot 自身が消す経路（/ticket delete・自動削除・撤去）は先に記録を消しているので、二重には消さない
- **Bot の停止中・外されていた間に消されたチャンネルは channelDelete が届かない。** 起動時と再導入時に、ギルドのチケットのうちチャンネルが既に無いものを片付けるようにした（`ticketChannelSync.ts`）。チャンネル一覧（`GET /guilds/{id}/channels`、閲覧権限に関係なく全チャンネルを返す）の取得に失敗したときは、存在しないと誤判定しないよう何も消さない
- **再導入時にチケットの自動削除タイマーを組み直すようにした**（v3.2.0 の既知の制限の解消）。これまでは次の再起動まで止まったままだった。タイマーの発火時はチャンネルが無くても DB の記録を消して終わるだけなので、外されていた間にチャンネルを整理されていてもエラーにはならない
- 残る制限: 切断中に再導入されたギルドは `guildCreate` が飛ばない（`guildAvailable` になる）ため、上の片付けと組み直しは次の起動まで行われない

### マニュアルの既存の誤りを直した（2026-09-26 完了・本番デプロイ済み）

v3.3.0。リリース前の点検で見つけた、以前からの誤り2件。`/guild-settings view` の説明が「ギルド設定と各機能の設定をページ形式で一覧表示」のままだった（実際は言語とエラー通知チャンネルの Embed 1枚だけ。遅くとも 2026-05-29 から）ので、各機能は `*-settings view` で見る旨とあわせて直した。導入時・再導入時のオーナー DM（v3.2.0）の説明が無かったので節を足した。導入時の DM は「初めて導入したとき」ではなく、削除予約を取り消さなかった導入すべて（30日を過ぎてから入れ直した場合も含む）で届く。
