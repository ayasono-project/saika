...
vi.mock("@/features/member-log/handlers/accountAge", () => ({
...
describe("Member Log Handlers Integration", () => {
  ...
  describe("guildMemberAdd: 参加通知", () => {
    async function loadHandler() {
      return (
        await import("@/features/member-log/handlers/guildMemberAddHandler")
    ...
    it("招待リンクの使用回数が増加した場合に招待者情報をEmbedに含めること", async () => {
      ...
      const { clearInviteCacheForTest } = await import(
        "@/features/member-log/handlers/inviteTracker"
      );
  ...
  describe("guildMemberRemove: 退出通知", () => {
    async function loadHandler() {
      return (
        await import("@/features/member-log/handlers/guildMemberRemoveHandler")
