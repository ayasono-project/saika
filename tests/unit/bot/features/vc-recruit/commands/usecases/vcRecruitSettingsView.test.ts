// tests/unit/bot/features/vc-recruit/commands/usecases/vcRecruitSettingsView.test.ts

import { ValidationError } from "@ayasono/shared/core";
import { handleVcRecruitSettingsView } from "@/bot/features/vc-recruit/commands/usecases/vcRecruitSettingsView";
...
describe("bot/features/vc-recruit/commands/usecases/vcRecruitSettingsView", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("guild が null の場合は ValidationError を投げる", async () => {
    ...
    await expect(
      handleVcRecruitSettingsView(interaction as never, GUILD_ID),
  ...
  it("セットアップ・ロールが空の場合もエラーなく info embed で返信する", async () => {
    ...
    await handleVcRecruitSettingsView(interaction as never, GUILD_ID);
  ...
  it("セットアップがある場合はカテゴリー名を含む返信をする（既知カテゴリーあり）", async () => {
    ...
    expect(getVcRecruitSettingsOrDefaultMock).toHaveBeenCalledWith(GUILD_ID);
  });
