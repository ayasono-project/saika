// tests/unit/bot/features/ticket/commands/usecases/ticketSettingsView.test.ts
...
describe("bot/features/ticket/commands/usecases/ticketSettingsView", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("設定が存在しない場合はinfo応答を返す", async () => {
    const { handleTicketSettingsView } = await import(
      "@/bot/features/ticket/commands/usecases/ticketSettingsView"
    );

    findAllByGuildMock.mockResolvedValue([]);
    const interaction = createInteractionMock();

    await handleTicketSettingsView(interaction as never, "guild-1");
...
describe("buildSettingsEmbed", () => {
  ...
  it("設定情報からEmbedを正しく生成する", async () => {
    const { buildSettingsEmbed } = await import(
      "@/bot/features/ticket/commands/usecases/ticketSettingsView"
    );

    const config = createConfig("cat-1");
    const embed = buildSettingsEmbed(config as never, 5, "ja");
