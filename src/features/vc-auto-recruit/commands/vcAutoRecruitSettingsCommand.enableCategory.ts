// src/features/vc-auto-recruit/commands/vcAutoRecruitSettingsCommand.enableCategory.ts
// vc-auto-recruit-settings enable-category 実行処理
...
/**
 * 募集対象カテゴリ（allowlist）を有効化する。カテゴリ省略時は TOP（ルート直下）を有効化する
 * @param interaction コマンド実行インタラクション
 * @param guildId 設定更新対象のギルドID
 * @returns 実行完了を示す Promise
 */
export async function handleVcAutoRecruitSettingsEnableCategory(
  ...
  const added = await getBotVcAutoRecruitSettingsService().addEnabledCategory(
    guildId,
    categoryKey,
  );

  // 既に有効な場合は情報メッセージを返す（冪等）
  if (!added) {
    await interaction.reply({
      embeds: [
        createInfoEmbed(
          tInteraction(
            locale,
            "vcAutoRecruit:user-response.category_already_enabled",
  ...
  await interaction.reply({
    embeds: [
      createSuccessEmbed(
        tInteraction(locale, "vcAutoRecruit:user-response.category_enabled", {
          category: categoryLabel,
        }),
  ...
  logger.info(
    logPrefixed(
      ...
      "vcAutoRecruit:log.config_category_enabled",
