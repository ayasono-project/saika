// tests/unit/bot/features/guild-settings/commands/guildSettingsCommand.import.test.ts
...
import { handleImport } from "@/bot/features/guild-settings/commands/guildSettingsCommand.import";
import { GUILD_SETTINGS_CUSTOM_ID } from "@/bot/features/guild-settings/constants/guildSettings.constants";
...
describe("bot/features/guild-settings/commands/guildSettingsCommand.import", () => {
    ...
    expect(interaction.reply).toHaveBeenCalledWith(
      expect.objectContaining({
        embeds: [
          {
            kind: "error",
            description: "guildSettings:user-response.import_invalid_json",
          },
        ],
      }),
    );
  ...
  it("バリデーション通過時は確認ダイアログが表示されること", async () => {
    const data = { version: 1, guildId: "guild-1", settings: { locale: "ja" } };
    fetchMock.mockResolvedValue({
  ...
  it("確認ボタン押下で importSettings と invalidateLocaleCache が呼ばれること", async () => {
    const data = { version: 1, guildId: "guild-1", settings: { locale: "ja" } };
    fetchMock.mockResolvedValue({
      text: () => Promise.resolve(JSON.stringify(data)),
    });
    ...
    const buttonInteraction = {
      customId: GUILD_SETTINGS_CUSTOM_ID.IMPORT_CONFIRM,
      user: { id: "user-1" },
  ...
  it("キャンセルボタン押下で importSettings が呼ばれないこと", async () => {
    const data = { version: 1, guildId: "guild-1", settings: { locale: "ja" } };
    fetchMock.mockResolvedValue({
      text: () => Promise.resolve(JSON.stringify(data)),
    });
    ...
    const buttonInteraction = {
      customId: GUILD_SETTINGS_CUSTOM_ID.IMPORT_CANCEL,
      user: { id: "user-1" },
      update: vi.fn().mockResolvedValue(undefined),
    };
    await collectCb.collect(buttonInteraction);
  ...
  it("チャンネル/ロールが不在の場合は警告付きで確認ダイアログが表示されること", async () => {
    const data = {
      version: 1,
      guildId: "guild-1",
      settings: { locale: "ja", errorChannelId: "missing-ch" },
    };
    fetchMock.mockResolvedValue({
      text: () => Promise.resolve(JSON.stringify(data)),
    });
  ...
  it("確認ダイアログに stateful 新規追加予定件数のフィールドが追加されること", async () => {
    ...
    expect(replyArgs.embeds[0]?.fields[1]?.name).toBe(
      "guildSettings:embed.field.name.import_state",
    );
  });
