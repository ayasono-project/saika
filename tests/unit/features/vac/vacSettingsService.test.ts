...
describe("shared/features/vac/vacSettingsService", () => {
  const createRepositoryMock = () => ({
    getVacSettings: vi.fn<() => Promise<VacSettings | null>>(),
    updateVacSettings:
      vi.fn<(guildId: string, config: VacSettings) => Promise<void>>(),
  });
  ...
  const makeConfig = (overrides: Partial<VacSettings> = {}): VacSettings => ({
    enabled: true,
    triggerChannelIds: [],
    createdChannels: [],
    ...overrides,
  ...
  it("設定が未登録の場合はデフォルト値を返し、呼び出しごとに別インスタンスになること", async () => {
    ...
    const first = await service.getVacSettingsOrDefault("g1");
    const second = await service.getVacSettingsOrDefault("g1");

    expect(first).toEqual(DEFAULT_VAC_SETTINGS);
  ...
  it("設定が存在する場合は内容が一致した正規化コピーを返し、配列は元と別参照であること", async () => {
    ...
    repository.getVacSettings.mockResolvedValue(raw);
    const service = new VacSettingsService(repository);

    const config = await service.getVacSettingsOrDefault("g1");
  ...
  it("保存時に正規化済みコピーが repository へ渡され、入力オブジェクトとは別参照であること", async () => {
    const repository = createRepositoryMock();
    repository.updateVacSettings.mockResolvedValue(undefined);
    const service = new VacSettingsService(repository);

    const input = makeConfig({ triggerChannelIds: ["ch-1"] });
    await service.saveVacSettings("g1", input);

    expect(repository.updateVacSettings).toHaveBeenCalledWith(
      "g1",
      expect.objectContaining({ triggerChannelIds: ["ch-1"] }),
    );
    // 渡されたオブジェクトが元と別参照であること（正規化コピー）
    const saved = repository.updateVacSettings.mock.calls[0][1] as VacSettings;
  ...
  it("新規チャンネルを追加すると一覧に含まれ、機能が有効化されること", async () => {
    ...
    expect(repository.updateVacSettings).toHaveBeenCalledOnce();
  });
  ...
  it("すでに登録済みのチャンネルを追加しても重複せず、保存が呼ばれないこと", async () => {
    ...
    // 変更なしのため保存は呼ばれない
    expect(repository.updateVacSettings).not.toHaveBeenCalled();
  });
  ...
  it("createVacSettingsService は VacSettingsService のインスタンスを返すこと", async () => {
    const { createVacSettingsService } = await import(
      "@/features/vac/vacSettingsService"
    );
    const repository = createRepositoryMock();
    const service = createVacSettingsService(repository);
    expect(service).toBeInstanceOf(
      (await import("@/features/vac/vacSettingsService")).VacSettingsService,
    );
  });
});
