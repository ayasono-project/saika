 ...
 */
export const bumpReminderSettingsCommand: Command = {
  data: (() => {
    ...
    const enableDesc = getCommandLocalizations(
      "bumpReminder",
      "bump-reminder-settings.enable.description",
    );
    const disableDesc = getCommandLocalizations(
      "bumpReminder",
      "bump-reminder-settings.disable.description",
    );
    const setMentionDesc = getCommandLocalizations(
      "bumpReminder",
      "bump-reminder-settings.set-mention.description",
    );
    ...
    const resetDesc = getCommandLocalizations(
      "bumpReminder",
      "bump-reminder-settings.reset.description",
    );
    const viewDesc = getCommandLocalizations(
      "bumpReminder",
      "bump-reminder-settings.view.description",
    );
  ...
  /**
   * bump-reminder-settings コマンド実行を features 側へ委譲する
   * @param interaction コマンド実行インタラクション
   * @returns 実行完了を示す Promise
   */
  async execute(interaction) {
    try {
      await executeBumpReminderSettingsCommand(interaction);
    } catch (error) {
