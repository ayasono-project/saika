// tests/unit/bot/features/reaction-role/commands/usecases/reactionRoleSettingsEditPanel.test.ts
...
vi.mock("@/bot/services/botCompositionRoot", () => ({
  getBotReactionRolePanelSettingsService: vi.fn(() => ({
...
describe("bot/features/reaction-role/commands/usecases/reactionRoleSettingsEditPanel", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("パネルが0件の場合はエラー応答を返す", async () => {
    const { handleReactionRoleSettingsEditPanel } = await import(
    ...
    await handleReactionRoleSettingsEditPanel(interaction as never, "guild-1");
