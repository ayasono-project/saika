...
// guild-settings コマンド定義・ルーティングを検証
describe("bot/commands/guild-settings", () => {
  // 各ケースでモック呼び出し記録をリセットし、テスト間の副作用を排除する
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("コマンド名が guild-settings であること", () => {
    expect(guildSettingsCommand.data.name).toBe("guild-settings");
  });

  it("ManageGuild 権限がデフォルトで要求されていること", () => {
    expect(guildSettingsCommand.data.default_member_permissions).toBe(
