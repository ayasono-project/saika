...
### 次のリリースで saika を 3.0.0 に上げる（2026-09-20 決定）

非アクティブ自動キックと `/vc` という機能2つを削除し、`/afk` の仕様も変えるため、ユーザーから見て破壊的変更にあたる。バージョンは `/about` に表示される（[`appVersion.ts`](src/shared/config/appVersion.ts) が `package.json` を読む）ので、実際に目に入る。

`@ayasono/shared` を同じ理由で v2.0.0 に上げた直後にこちらをマイナーで出すと基準がぶれるため揃えた。2.3.0 からの連番ではなくメジャーを飛ばす点は意図的。
...
### Bot ステータスをギルド参加・退出時に更新する（2026-09-20 develop merge）
...
### 開発 Bot に残るグローバルコマンド残骸の恒久対応（2026-09-20 develop merge）
