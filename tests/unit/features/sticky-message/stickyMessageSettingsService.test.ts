...
// StickyMessageSettingsService の各メソッドがリポジトリへ正しい引数を委譲するか、
// およびシングルトン管理関数（set/get/create）のライフサイクルが正しく機能するかを検証する
describe("shared/features/sticky-message/stickyMessageSettingsService", () => {
  ...
  async function loadModule() {
    return import("@/features/sticky-message/stickyMessageSettingsService");
  }
  ...
  it("findByChannel がリポジトリへ委譲すること", async () => {
    ...
    const { StickyMessageSettingsService } = await loadModule();
    const service = new StickyMessageSettingsService(repo as never);
  ...
  it("createStickyMessageSettingsService が新しいサービスインスタンスを返すこと", async () => {
    const repo = createRepoMock();
    const { createStickyMessageSettingsService, StickyMessageSettingsService } =
      await loadModule();

    const service = createStickyMessageSettingsService(repo as never);
    expect(service).toBeInstanceOf(StickyMessageSettingsService);
  });
});
