...
} from "@/shared/features/ticket/ticketSettingsService";
...
// TicketSettingsService が各メソッドを IGuildTicketSettingsRepository へ正しく委譲することを検証する
describe("shared/features/ticket/ticketSettingsService", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("createTicketSettingsService ファクトリ関数がインスタンスを生成すること", () => {
    const repository = createRepositoryMock();
    const service = createTicketSettingsService(repository as never);

    expect(service).toBeInstanceOf(TicketSettingsService);
  });
});
