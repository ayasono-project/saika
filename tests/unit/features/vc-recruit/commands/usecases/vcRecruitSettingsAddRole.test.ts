// tests/unit/bot/features/vc-recruit/commands/usecases/vcRecruitSettingsAddRole.test.ts

import { ValidationError } from "@ayasono/shared/core";
import { handleVcRecruitSettingsAddRole } from "@/features/vc-recruit/commands/usecases/vcRecruitSettingsAddRole";
...
// handleVcRecruitSettingsAddRole のギルドチェック・RoleSelectMenu 返信・customId 構造を検証
describe("bot/features/vc-recruit/commands/usecases/vcRecruitSettingsAddRole", () => {
  ...
  it("guild が null の場合は ValidationError を投げる", async () => {
    ...
    await expect(
      handleVcRecruitSettingsAddRole(interaction as never),
    ).rejects.toBeInstanceOf(ValidationError);
  });

  it("RoleSelectMenu とボタンをエフェメラルで返信する", async () => {
    const interaction = makeInteraction();
    await handleVcRecruitSettingsAddRole(interaction as never);
