// tests/unit/bot/features/vac/commands/usecases/vacSettingsRemoveTrigger.test.ts
...
describe("bot/features/vac/commands/usecases/vacSettingsRemoveTrigger", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("ギルドコンテキストが存在しない場合にValidationErrorをスローする", async () => {
    ...
    await expect(
      handleVacSettingsRemoveTrigger(interaction as never, "guild-1"),
    ).rejects.toBeInstanceOf(ValidationError);
  });

  it("トリガーチャンネルが0件の場合にValidationErrorをスローする", async () => {
    getVacSettingsOrDefaultMock.mockResolvedValue({
      triggerChannelIds: [],
    });
  ...
  it("トリガーチャンネルがある場合にセレクトメニュー付きで reply する", async () => {
    ...
    await handleVacSettingsRemoveTrigger(interaction as never, "guild-1");
  ...
  it("セレクトメニューで選択して削除ボタンで削除が実行される", async () => {
    ...
    const selectInteraction = {
      customId: "vac-settings:trigger-remove-select",
    ...
    const confirmInteraction = {
      customId: "vac-settings:trigger-remove-confirm",
