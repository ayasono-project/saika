...
describe("bot/features/ticket/commands/usecases/ticketSettingsSetMaxTickets", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("設定が見つからない場合はエラー応答", async () => {
    const { handleTicketSettingsSetMaxTickets } = await import(
    ...
    await handleTicketSettingsSetMaxTickets(interaction as never, "guild-1");
