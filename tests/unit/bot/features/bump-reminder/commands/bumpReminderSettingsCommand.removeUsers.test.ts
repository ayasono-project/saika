...
import { handleBumpReminderSettingsRemoveUsers } from "@/bot/features/bump-reminder/commands/bumpReminderSettingsCommand.removeUsers";

const getBumpReminderSettingsOrDefaultMock = vi.fn();
...
vi.mock("@/bot/services/botCompositionRoot", () => ({
  getBotBumpReminderSettingsService: () => ({
    getBumpReminderSettingsOrDefault: (...args: unknown[]) =>
      getBumpReminderSettingsOrDefaultMock(...args),
...
describe("bumpReminderSettingsCommand.removeUsers", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("登録ユーザーが0人の場合は ValidationError を投げる", async () => {
    getBumpReminderSettingsOrDefaultMock.mockResolvedValue({
      enabled: true,
      mentionUserIds: [],
    });
    ...
    await expect(
      handleBumpReminderSettingsRemoveUsers(interaction as never, "guild-1"),
  ...
  it("登録ユーザーがある場合にセレクトメニュー付きで reply する", async () => {
    ...
    await handleBumpReminderSettingsRemoveUsers(
  ...
  it("セレクトメニューで選択した後に削除ボタンで削除が実行される", async () => {
    ...
    expect(saveBumpReminderSettingsMock).toHaveBeenCalledWith(
      "guild-1",
      expect.objectContaining({
  ...
  it("全員選択ボタンで全ユーザーが選択される", async () => {
    ...
    expect(saveBumpReminderSettingsMock).toHaveBeenCalledTimes(1);
    expect(collectorMock.stop).toHaveBeenCalled();
  });
