...
> 色ラベル・説明・プレースホルダー（`Select a color`）はロケール非依存の英語固定。ハードコードは避け、定数ファイル [`reactionRoleCommand.constants.ts`](../../src/bot/features/reaction-role/commands/reactionRoleCommand.constants.ts) で一元管理する（例: `REACTION_ROLE_BUTTON_COLORS` 配列 + `REACTION_ROLE_COLOR_PLACEHOLDER`）。SelectMenu のオプションはこの定数を `map` してビルドする。
