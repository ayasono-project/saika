...
import { vacSettingsCommand } from "@/bot/commands/vac-settings";
...
// vac-settings 実行用 interaction モック
...
// vac-settings autocomplete 用 interaction モック
...
// vac-settings コマンドの各サブコマンド（create-trigger-vc / remove-trigger-vc / view）と
// オートコンプリートについて、権限チェック／カテゴリ解決／DB操作／応答内容を検証する
describe("bot/commands/vac-settings", () => {
  // ケース間でモック状態を初期化する
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("ManageGuild 権限がない場合に権限チェックエラーが handleCommandError へ委譲されることを確認", async () => {
    ...
    await vacSettingsCommand.execute(
  ...
  it("オートコンプリートで TOP と一致するカテゴリ候補が返されることを確認", async () => {
    ...
    await vacSettingsCommand.autocomplete!(
