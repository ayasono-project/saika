...
| `/reaction-role-settings setup` | パネル作成（Embed設定 → モード選択 → ボタン追加 → 設置） |
| `/reaction-role-settings teardown` | パネル撤去（複数選択可） |
| `/reaction-role-settings view` | 設定一覧表示 |
| `/reaction-role-settings edit-panel` | パネルのタイトル・説明文・カラー編集 |
| `/reaction-role-settings add-button` | 既存パネルにボタン追加 |
| `/reaction-role-settings remove-button` | パネルからボタン削除 |
| `/reaction-role-settings edit-button` | ボタンのラベル・絵文字・色・ロール変更 |
...
setup / add-button / edit-button の RoleSelectMenu で選択されたロールは、以下のいずれかに該当する場合に設定を拒否して [`user-response.role_above_hierarchy`](#ローカライズ) エラーメッセージ（該当ロールメンション一覧を含む）を返す。拒否時はセッション・DB への保存を行わず、ユーザーを直前の選択 UI に戻す。

- Bot の最上位ロールの position **以上** の position を持つロール（Bot が付与・解除できないため）
- コマンド実行者（member）の最上位ロールの position **以上** の position を持つロール（Discord の通常権限モデルに揃える）

**例外:**

- コマンド実行者がサーバーオーナー（`interaction.guild.ownerId === interaction.user.id`）の場合は、実行者階層チェックを免除する（Discord 仕様上オーナーは階層制約を受けないため）。Bot 階層チェックは常に適用する。
- `@everyone` ロールは RoleSelectMenu の選択肢に含まれないため考慮不要。

**Why:** Discord の仕様上、Bot に Administrator 権限を付与してもロール階層の制約は回避できない。click 時のエラー（`user-response.role_too_high`）で初めて発覚する UX を避け、setup 段階で未然にブロックする。
...
**コマンド**: `/reaction-role-settings setup`
...
4. ボタン追加ループ:
   a. モーダルを表示（ボタンラベル・絵文字入力）
   b. 色選択 StringSelectMenu を表示（`primary` をデフォルト選択）
   c. RoleSelectMenu を表示（割り当てロール選択、複数選択可）
   d. 「もう1つ追加」「完了」ボタンを表示
   e. 「もう1つ追加」→ 4a に戻る / 「完了」→ 5 へ
...
- ボタン色は省略時 `primary`（色選択 SelectMenu で `primary` を default 選択）
- 選択ロールのロール階層バリデーション（後述「ロール階層バリデーション（共通）」節参照）
...
> モーダル内には StringSelectMenu を配置できない Discord の制約のため、ボタン色はモーダル送信後に別 UI（色選択 StringSelectMenu）で指定する。

**色選択 StringSelectMenu:**

| コンポーネント | スタイル | 動作 |
| --- | --- | --- |
| `reaction-role:setup-color:<sessionId>` | StringSelect（単一選択、`primary` を default 選択） | ボタンの色を選択 |

**色選択肢（共通）:**

| ラベル | 値（DB 保存値） | 説明 |
| --- | --- | --- |
| Blue | `primary` | Discord Primary button |
| Gray | `secondary` | Discord Secondary button |
| Green | `success` | Discord Success button |
| Red | `danger` | Discord Danger button |

> 色ラベル・説明・プレースホルダー（`Select a color`）はロケール非依存の英語固定。ハードコードは避け、定数ファイル [`reactionRoleCommand.constants.ts`](../../src/features/reaction-role/commands/reactionRoleCommand.constants.ts) で一元管理する（例: `REACTION_ROLE_BUTTON_COLORS` 配列 + `REACTION_ROLE_COLOR_PLACEHOLDER`）。SelectMenu のオプションはこの定数を `map` してビルドする。
...
1. ボタンが乗っているメッセージ（`interaction.message.id` = パネルの `messageId`）でパネル設定を DB から取得する。customId に含まれる `panelId`(cuid) は使わない（DB 移行で id が再生成されても壊れないようにするため）
2. クリックされたボタンの割り当てロールを特定（`buttonId` は customId から取得）
3. モードに応じた処理を実行（下記参照）
...
**コマンド**: `/reaction-role-settings teardown`
...
**コマンド**: `/reaction-role-settings view`
...
**コマンド**: `/reaction-role-settings edit-panel`
...
**コマンド**: `/reaction-role-settings add-button`
...
4. ボタン追加ループ:
   ...
   c. RoleSelectMenu を表示（割り当てロール選択、複数選択可）
   d. 「もう1つ追加」「完了」ボタンを表示
   e. 「もう1つ追加」→ 上限チェック後 4a に戻る / 「完了」→ 5 へ
5. パネルメッセージを更新（ボタン追加）
6. 設定を DB に保存
...
| コンポーネント | スタイル | 動作 |
| --- | --- | --- |
| `reaction-role:add-button-color:<sessionId>` | StringSelect（単一選択、`primary` を default 選択） | ボタンの色を選択 |

選択肢は setup の「色選択肢（共通）」と同じ。
...
**コマンド**: `/reaction-role-settings remove-button`
...
**コマンド**: `/reaction-role-settings edit-button`
...
4. モーダルを表示（現在の設定値をプリフィル: ラベル・絵文字）
5. 色選択 StringSelectMenu を表示（現在の色が default 選択された状態）
6. RoleSelectMenu を表示（現在のロールが選択された状態で表示、複数選択可）
7. パネルメッセージを更新
8. 設定を DB に保存
...
- ロールは1つ以上選択必須
- 色は現在値を default 選択しておき、変更しない場合は既存値のまま維持
- 選択ロールのロール階層バリデーション（後述「ロール階層バリデーション（共通）」節参照）
...
| コンポーネント | スタイル | 動作 |
| --- | --- | --- |
| `reaction-role:edit-button-color:<sessionId>` | StringSelect（単一選択、現在の色を default 選択） | ボタンの色を変更 |
...
| `buttons` | Json | ボタン設定（jsonb 配列: `ReactionRoleButton[]`） |
...
### ダッシュボード（API）経由削除と孤児化通知

Web ダッシュボードからのパネル削除（API: `DELETE /:guildId/reaction-roles/:id`）では、bot の messageDelete / channelDelete ハンドラと**同一パネルを複数経路から削除しうる**ため、以下の設計を採る。

**削除順序とレース耐性:**

1. `findById` で対象パネルの存在とギルド一致を確認
2. DB レコードを先に削除 → その後 Discord メッセージを削除（`deletePanelMessage`）
   - メッセージ削除を先にすると bot の messageDelete ハンドラが先に DB レコードを掃除し、後続の削除が「対象なし」で失敗するため、DB を先に消す
3. リポジトリの削除は **冪等**（`deleteMany` を使用し、0 件マッチでも例外を投げない）。ダッシュボードの二重削除や、messageDelete/channelDelete ハンドラとの TOCTOU レースで負けた側も正常終了として扱う（`PrismaClientKnownRequestError P2025` を出さない）

**孤児化リスクの通知（`deletePanelMessage`）:**

DB 行削除後に Discord メッセージ削除が失敗すると「DB 行なし・メッセージ残存」の孤児パネルになる。これを握りつぶさず二経路で可視化する:

| 宛先 | レベル | 手段 | 用途 |
| --- | --- | --- | --- |
| 運営者 webhook | error | `logger.error`（`log.panel_message_delete_failed`） | 運営の横断監視（webhook は level:error のみ捕捉） |
| ギルドのエラーチャンネル | warn | `notifyWarnChannel`（`errorChannelId` 設定時） | 当事者である管理者へ手動削除/権限確認を促す |

- チャンネル取得不可/非テキスト、またはメッセージ削除失敗（権限不足等）= 孤児リスク → 両経路へ通知
- メッセージが既に削除済み（Discord APIerror code `10008` = Unknown Message）= 望ましい終状態なので通知しない（誤検知ノイズ回避）
- 既存の孤児パネルの遡及クリーンアップ・リコンサイルはスコープ外（新規発生の観測まで）

> 設計メモ: `deletePanelMessage` は discord.js を API の静的 import グラフに載せない方針のため、`errorChannelNotifier` は動的 import で呼び、`DiscordAPIError` 型を import せず数値判定（`err.code === 10008`）で既消みを除外する。
...
| `log.panel_message_delete_failed` | パネルメッセージ削除失敗（孤児化）ログ | パネルメッセージ削除に失敗（孤児化の可能性） GuildId: {{guildId}} PanelId: {{panelId}} ChannelId: {{channelId}} MessageId: {{messageId}} | failed to delete panel message (possible orphan) GuildId: {{guildId}} PanelId: {{panelId}} ChannelId: {{channelId}} MessageId: {{messageId}} |
...
| キー | 用途 | ja | en |
| --- | --- | --- | --- |
| `reaction-role-settings.description` | コマンド説明 | リアクションロール機能の設定（サーバー管理権限が必要） | Configure reaction role feature (requires Manage Server) |
| `reaction-role-settings.setup.description` | サブコマンド説明 | リアクションロールパネルを設置 | Set up reaction role panel |
| `reaction-role-settings.teardown.description` | サブコマンド説明 | リアクションロールパネルを撤去 | Remove reaction role panel |
| `reaction-role-settings.view.description` | サブコマンド説明 | 現在の設定を表示 | Show current settings |
| `reaction-role-settings.edit-panel.description` | サブコマンド説明 | パネルのタイトル・説明文・カラーを編集 | Edit panel title, description, and color |
| `reaction-role-settings.add-button.description` | サブコマンド説明 | パネルにボタンを追加 | Add a button to panel |
| `reaction-role-settings.remove-button.description` | サブコマンド説明 | パネルからボタンを削除 | Remove a button from panel |
| `reaction-role-settings.edit-button.description` | サブコマンド説明 | ボタンを編集 | Edit a button |
...
| `user-response.role_above_hierarchy` | ロール階層エラー（setup時） | 以下のロールはBotまたはあなたの最上位ロールより上位のため設定できません: {{roles}} | The following roles are above the bot's or your highest role and cannot be configured: {{roles}} |
...
> 色選択 SelectMenu のラベル・説明・プレースホルダーは i18n 対象外（英語固定）。詳細は「色選択肢（共通）」表および定数ファイルを参照。
...
- [ ] setup: モーダル入力、モード選択、色選択 SelectMenu（default=`primary`）、ボタン追加ループ、パネル送信+DB保存、Bot権限不足エラー伝播
- [ ] teardown: パネル選択（複数選択可）、確認ダイアログ、正常削除（単数・複数）
- [ ] view: ページネーション、パネル選択
- [ ] edit-panel: モーダルでタイトル・説明文・カラー編集
- [ ] add-button: パネル選択、モーダル、色選択 SelectMenu、ロール選択、追加ループ
- [ ] remove-button: ボタン選択・削除
- [ ] edit-button: モーダル、色選択 SelectMenu（default=現在の色）、ロール選択の順で編集
- [ ] ロール操作: ボタンクリック → toggle / one-action / exclusive モード
- [ ] ロール階層バリデーション: Bot 上位ロール拒否 / 実行者上位ロール拒否 / サーバーオーナー実行時は実行者階層チェック免除 / 複数ロール混在時は NG ロールのみリストアップ（setup / add-button / edit-button の 3 フローで同一挙動）
