// tests/unit/bot/features/member-log/commands/memberLogSettingsCommand.clearJoinMessage.test.ts

import { ValidationError } from "@ayasono/shared/core";
import { handleMemberLogSettingsClearJoinMessage } from "@/bot/features/member-log/commands/memberLogSettingsCommand.clearJoinMessage";
...
// handleMemberLogSettingsClearJoinMessage の権限チェック・削除・応答フローを検証
describe("bot/features/member-log/commands/memberLogSettingsCommand.clearJoinMessage", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("ガードが ValidationError を投げた場合にそれが伝播することを確認", async () => {
    ...
    await expect(
      handleMemberLogSettingsClearJoinMessage(
  ...
  it("service.clearJoinMessage が正しい guildId を引数に呼ばれることを確認", async () => {
    ...
    await handleMemberLogSettingsClearJoinMessage(
      makeInteraction() as never,
      "guild-1",
    );
