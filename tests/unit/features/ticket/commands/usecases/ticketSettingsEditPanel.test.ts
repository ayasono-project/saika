// tests/unit/bot/features/ticket/commands/usecases/ticketSettingsEditPanel.test.ts
...
describe("bot/features/ticket/commands/usecases/ticketSettingsEditPanel", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("設定が見つからない場合はエラー応答を返す", async () => {
    const { handleTicketSettingsEditPanel } = await import(
      "@/features/ticket/commands/usecases/ticketSettingsEditPanel"
    );

    findByGuildAndCategoryMock.mockResolvedValue(null);
    const interaction = createInteractionMock();

    await handleTicketSettingsEditPanel(interaction as never, "guild-1");
