...
**デフォルト状態:** `enabled: false` / `channelId: null` / `thresholdDays: 30` / `warnMessage: null` / `kickMessage: null` / `markerRoleId: null` / `whitelistRoleIds: []` / `whitelistUserIds: []` / `enabledAt: null`。設定が無くても成功扱い（冪等）。`MemberActivity` は reset では削除しない（活動履歴は設定リセットと独立。再有効化時に活かせる）。
