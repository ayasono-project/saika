// tests/unit/bot/features/ticket/commands/usecases/ticketSettingsAddRoles.test.ts
...
describe("bot/features/ticket/commands/usecases/ticketSettingsAddRoles", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("設定が見つからない場合はエラー応答を返す", async () => {
    const { handleTicketSettingsAddRoles } = await import(
      "@/features/ticket/commands/usecases/ticketSettingsAddRoles"
    );

    findByGuildAndCategoryMock.mockResolvedValue(null);
    const interaction = createInteractionMock();

    await handleTicketSettingsAddRoles(interaction as never, "guild-1");
