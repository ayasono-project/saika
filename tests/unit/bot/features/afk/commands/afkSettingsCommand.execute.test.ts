...
import { executeAfkSettingsCommand } from "@/bot/features/afk/commands/afkSettingsCommand.execute";
...
// afkSettingsCommand の実行ロジックが
// 権限チェック・サブコマンド分岐（set-channel / view）を正しく処理するかを検証する
describe("bot/features/afk/commands/afkSettingsCommand.execute", () => {
  ...
  it("ManageGuild 権限を持たないメンバーがコマンドを実行した場合は ValidationError を投げる", async () => {
    ...
    await expect(
      executeAfkSettingsCommand(interaction as never),
  ...
  it("set-channel サブコマンドで AFK チャンネルを設定し成功返信する", async () => {
    ...
    await executeAfkSettingsCommand(interaction as never);
  ...
  it("view サブコマンドで現在の設定を表示する", async () => {
    ...
    expect(getAfkSettingsMock).toHaveBeenCalledWith("guild-1");
  ...
  it("clear-channel サブコマンドで AFK チャンネル設定を解除する", async () => {
    ...
    expect(saveAfkSettingsMock).toHaveBeenCalledWith("guild-1", {
      enabled: false,
      channelId: null,
    });
