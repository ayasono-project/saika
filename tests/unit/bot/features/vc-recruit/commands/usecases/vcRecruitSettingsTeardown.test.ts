// tests/unit/bot/features/vc-recruit/commands/usecases/vcRecruitSettingsTeardown.test.ts

import { ValidationError } from "@ayasono/shared/core";
import { handleVcRecruitSettingsTeardown } from "@/bot/features/vc-recruit/commands/usecases/vcRecruitSettingsTeardown";
import { VC_RECRUIT_TIMEOUT } from "@/bot/features/vc-recruit/commands/vcRecruitSettingsCommand.constants";
...
// handleVcRecruitSettingsTeardown のユースケースロジックを検証
describe("bot/features/vc-recruit/commands/usecases/vcRecruitSettingsTeardown", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("guild が null の場合は ValidationError を投げる", async () => {
    ...
    await expect(
      handleVcRecruitSettingsTeardown(interaction as never, GUILD_ID),
    ).rejects.toBeInstanceOf(ValidationError);
  });

  it("セットアップが0件の場合は ValidationError を投げる", async () => {
    getVcRecruitSettingsOrDefaultMock.mockResolvedValue({ setups: [] });
  ...
  it("セットアップが存在する場合は StringSelectMenu 付きのエフェメラルメッセージで応答する", async () => {
    ...
    const interaction = makeInteraction();
    await handleVcRecruitSettingsTeardown(interaction as never, GUILD_ID);
