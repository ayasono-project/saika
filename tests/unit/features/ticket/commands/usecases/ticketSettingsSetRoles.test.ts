// tests/unit/bot/features/ticket/commands/usecases/ticketSettingsSetRoles.test.ts
...
describe("bot/features/ticket/commands/usecases/ticketSettingsSetRoles", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("設定が見つからない場合はエラー応答を返す", async () => {
    const { handleTicketSettingsSetRoles } = await import(
      "@/features/ticket/commands/usecases/ticketSettingsSetRoles"
    );

    findByGuildAndCategoryMock.mockResolvedValue(null);
    const interaction = createInteractionMock();

    await handleTicketSettingsSetRoles(interaction as never, "guild-1");
