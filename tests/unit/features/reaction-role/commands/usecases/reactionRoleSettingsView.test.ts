// tests/unit/bot/features/reaction-role/commands/usecases/reactionRoleSettingsView.test.ts
...
describe("bot/features/reaction-role/commands/usecases/reactionRoleSettingsView", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("パネルが0件の場合はエラー応答を返す", async () => {
    const { handleReactionRoleSettingsView } = await import(
    ...
    await handleReactionRoleSettingsView(interaction as never, "guild-1");
