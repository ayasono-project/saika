...
describe("bot/features/reaction-role/commands/usecases/reactionRoleSettingsSetup", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("セッションが正しいデフォルト値で作成されること", async () => {
    const { handleReactionRoleSettingsSetup } = await import(
      "@/features/reaction-role/commands/usecases/reactionRoleSettingsSetup"
    );

    const interaction = createInteractionMock();

    await handleReactionRoleSettingsSetup(interaction as never, "guild-1");
