// src/shared/database/types/guildConfigExportTypes.ts
// guild-config export/import 用の stateful データ型定義
//
...
/** GuildTicketConfig の export 表現（staffRoleIds は配列に展開） */
export interface GuildTicketConfigExport {
  categoryId: string;
  enabled: boolean;
  staffRoleIds: string[];
...
/** stateful データの統合型（FullGuildConfig.state に格納される） */
export interface FullGuildState {
  ticketConfigs: GuildTicketConfigExport[];
