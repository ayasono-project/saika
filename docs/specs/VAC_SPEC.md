...
> **VC操作コマンド（名前変更・人数制限変更・切断・移動・AFK移動）について**: `/vc rename` / `/vc limit` / `/vc disconnect` / `/vc move` / `/afk` コマンドとして独立機能に切り出されています。詳細は [VC_COMMAND_SPEC.md](VC_COMMAND_SPEC.md) を参照してください。
>
> **操作パネルの廃止（2026-05-30）**: 作成VCのチャットに設置していた操作パネル（rename / limit / AFK / refresh ボタン）は廃止しました。全機能が public な slash コマンド（`/vc`・`/afk`）で代替可能であり、パネルは「チャットで流れる→refresh で再送」という構造上の問題と低利用率を抱えていたため。
...
| Bot | MoveMembers | ユーザーの自動移動（作成VCへの移動） |
...
> 作成されたVCの設定変更（名前・人数制限）・メンバーの切断/移動/AFK移動は、すべて `/vc`・`/afk` コマンドで行う。詳細は [VC_COMMAND_SPEC.md](VC_COMMAND_SPEC.md) / [AFK_SPEC.md](AFK_SPEC.md) を参照。
...
| [VC_COMMAND_SPEC.md](VC_COMMAND_SPEC.md) | 作成VCの設定変更・切断・移動は `/vc`・`/afk` コマンドが担当（VAC は管理対象VC判定のため `createdChannels` を提供） |
...
- [x] view: 設定表示、未設定表示
- [x] VC自動作成: トリガー検知、VC作成、権限付与、自動移動、DB保存、Bot権限不足時のエラーチャンネル通知
...
- [VC_COMMAND_SPEC.md](VC_COMMAND_SPEC.md) - VC操作コマンド仕様（名前変更・人数制限変更・切断・移動）
- [AFK_SPEC.md](AFK_SPEC.md) - AFK移動コマンド仕様
