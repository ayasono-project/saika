...
describe("bot/features/ticket/commands/usecases/ticketClose", () => {
  ...
  it("作成者でもスタッフでも管理者でもない場合は、誰がクローズできるか（スタッフロールのメンション入り）を 権限不足 で返信し、Bot の権限も確かめずクローズしない", async () => {
    const { handleTicketClose } = await import(
      "@/features/ticket/commands/usecases/ticketClose"
    );
  ...
  it("管理者権限（Administrator）があれば、スタッフロールが無く作成者でなくてもクローズできる", async () => {
    const { handleTicketClose } = await import(
      "@/features/ticket/commands/usecases/ticketClose"
    );
    ...
    expect(interaction.reply).toHaveBeenCalledWith({
      embeds: [
        { type: "success", description: "ticket:user-response.ticket_closed" },
      ],
      flags: MessageFlags.Ephemeral,
    });
  });

  it("作成者なら、スタッフロールも管理者権限も無くてもクローズできる", async () => {
    const { handleTicketClose } = await import(
      "@/features/ticket/commands/usecases/ticketClose"
    );
  ...
  it("Bot がチャンネルを扱えない（Bot を外して入れ直す前に作ったチケット）ときは、closeTicket を呼ばずに理由と対処を返信する", async () => {
    const { handleTicketClose } = await import(
      "@/features/ticket/commands/usecases/ticketClose"
    );
    ...
    // クローズはチャンネルを消さないので、「チャンネルの管理」は求めず4つの権限で確かめる
