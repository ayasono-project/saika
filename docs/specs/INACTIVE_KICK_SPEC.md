...
| set-warn-message / clear-warn-message | カスタム事前通知メッセージを設定・削除 |
...
通知と preview は**共通のページネーター**（Embed ページ + 前へ/次へボタン）を用いる。

- 1 Embed に収まる件数（Discord 制約: フィールド最大・1 フィールド 1024 文字・合計 6000 文字）ならボタンなしの単一 Embed。超過時のみページング。
...
| カラー | 黄系（警告） |
| 説明 | カスタム `warnMessage`（未設定時はデフォルト文）。変数: `{count}` / `{daysLeft}` / `{thresholdDays}` / `{serverName}` / `{markerRole}`（対象ロールメンション・設定時のみ） |
...
| カラー | 茜色（`#b7282d`） |
| 説明 | カスタム `kickMessage`（未設定時はデフォルト文）。変数: `{count}` / `{thresholdDays}` / `{serverName}` |
...
**ビジネスルール:** 範囲外はエラー。下限 14 日は、段階通知（7 日前・3 日前）が成立するための最小値（§9 の `graceDays` 1〜30 とは別物のため範囲も別建て）。デフォルト 60 日。

---

## /inactive-kick-settings set-warn-message / clear-warn-message

**コマンド**: `/inactive-kick-settings set-warn-message`（モーダル入力） / `clear-warn-message`

**モーダル:**

| フィールド | ラベル | スタイル | 必須 | 制約 |
| --- | --- | --- | --- | --- |
| `inactive-kick-settings:warn-message-modal-input` | 事前通知メッセージ | Paragraph | ✅ | 最大 500 文字 |

使用可能なプレースホルダー: `{count}` / `{daysLeft}` / `{thresholdDays}` / `{serverName}` / `{markerRole}`（対象ロール設定時のみ）
...
| `whitelist remove` | `role` / `user` | ホワイトリストから削除 |
| `whitelist list` | なし | 現在のホワイトリストを表示 |

**ビジネスルール:** `role` / `user` のいずれか必須。重複追加は冪等。
...
**デフォルト状態:** `enabled: false` / `channelId: null` / `thresholdDays: 60` / `warnMessage: null` / `kickMessage: null` / `markerRoleId: null` / `whitelistRoleIds: []` / `whitelistUserIds: []` / `enabledAt: null`。設定が無くても成功扱い（冪等）。`MemberActivity` は reset では削除しない（活動履歴は設定リセットと独立。再有効化時に活かせる）。
...
| `thresholdDays` | Int | 非アクティブ判定日数（デフォルト: 60） |
| `warnMessage` | String? | カスタム事前通知メッセージ |
...
- [ ] 非アクティブ日数の算出（レコードあり / なし=joinedAt 起算 / `enabledAt` による floor）
- [ ] 段階判定（1週間前 / 最終警告 / キック / warnStage による重複抑止 / 警告ゲート: `warnStage<2` はキックせず最終警告のみ）
- [ ] 除外判定（Bot / Administrator / ホワイトリスト role・user / オーナー / VC 接続中）
- [ ] 除外時の猶予クリア（warnStage>0 / 対象ロール付与済みの除外メンバーで warnStage=0 リセット＋ロール剥奪・VC 接続中も同様）
- [ ] 対象ロールの付与・剥奪判定（警告対象入りで付与 / 再活動で剥奪 / 階層不足でスキップ）
- [ ] 通知文の変数差し込み整形（warn / kick・単一波括弧）
- [ ] throttle による更新スキップ・再活動での warnStage リセット
- [ ] レコード掃除（キック時 / guildMemberRemove / guildDelete）
- [ ] 設定コマンド各種（set-threshold 範囲外・whitelist 冪等・enable で enabledAt 設定 等）

### インテグレーションテスト

- [ ] messageCreate / voiceStateUpdate / messageReactionAdd でのアクティビティ記録
- [ ] 日次チェック通し（`TEST_MODE=true`: キックせず通知のみ / 通常: キック実行）
- [ ] 警告ゲート通し（しきい値超過でも初回は最終警告のみ → 次回キック）
- [ ] 通知送信失敗時に `warnStage` を前進させない（H1: 次回再試行・キックしない）
- [ ] キック前に表示名を控えて通知に使う（H4: 退出後もメンバー名が残る）
- [ ] バリデーション失敗時の自動 `enabled=false`
- [ ] preview / 通知の共通ページネーター整形（単一 Embed / ページング）
- [ ] Bot 権限不足・キック不能メンバーのスキップ
...
| JobScheduler | 日次チェックの定期ジョブ（`addJob`・node-cron・`timezone` オプション。develop 反映済み） |
...
- [§9 未承認ユーザー自動キック](UNVERIFIED_KICK_SPEC.md)（日次チェック / バリデーション / preview の方式統一）
