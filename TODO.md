...
`guildDelete` 時に即削除せず、猶予後に削除する。詳細はタスク 3。猶予日数と Guild 親テーブルの採否は未決（下記）。
...
### 3. 案D（遅延削除）＋ guildCreate ハンドラ ＋ 導入時／再導入時の通知 【実装】
...
### 5. export / import の削除 【実装】

**タスク 3（案D）の完了待ち。** 順序を逆にしないこと。
...
### 6. Phase 6-1: `deleteAllSettings` のレジストリ化 【実装・条件付き】
...
> export/import 廃止に伴うマニュアル修正は**タスク 5 に同梱**する（別タイミングで走るため分離）。

### 8. ドキュメント整理（spec 廃止・guides 集約）
...
### 10. メンバーログの join/leave 出力先分離 【機能改善】

### 11. メッセージ出力機能 【機能追加】

### 12. ダッシュボード 【UI層・コアに影響しないので優先度低】
