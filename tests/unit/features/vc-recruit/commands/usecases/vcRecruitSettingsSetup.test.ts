// tests/unit/bot/features/vc-recruit/commands/usecases/vcRecruitSettingsSetup.test.ts

import { ValidationError } from "@ayasono/shared/core";
import { ChannelType, DiscordAPIError, RESTJSONErrorCodes } from "discord.js";
import { handleVcRecruitSettingsSetup } from "@/features/vc-recruit/commands/usecases/vcRecruitSettingsSetup";
...
vi.mock(
  "@/features/vc-recruit/commands/helpers/vcRecruitTargetResolver",
  () => ({
...
describe("bot/features/vc-recruit/commands/usecases/vcRecruitSettingsSetup", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("guild が null の場合は ValidationError を投げる", async () => {
    ...
    await expect(
      handleVcRecruitSettingsSetup(interaction as never, GUILD_ID),
  ...
  it("カテゴリーなし（TOP）で正常にセットアップでき、パネル/投稿チャンネルを作成して DB に保存する", async () => {
    const guild = makeGuild();
    const interaction = makeInteraction({ guildObj: guild });
    await handleVcRecruitSettingsSetup(interaction as never, GUILD_ID);
