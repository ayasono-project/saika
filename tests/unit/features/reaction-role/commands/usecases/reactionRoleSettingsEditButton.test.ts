...
describe("bot/features/reaction-role/commands/usecases/reactionRoleSettingsEditButton", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("パネルが0件の場合はエラー応答を返す", async () => {
    const { handleReactionRoleSettingsEditButton } = await import(
    ...
    await handleReactionRoleSettingsEditButton(interaction as never, "guild-1");
