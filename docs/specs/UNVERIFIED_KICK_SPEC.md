...
判定は **単一条件**（参加経過日数・認証ロール未取得・Bot 以外・除外ロール非保持）で行い、認証フローの個別ステージ（プロフィール投稿履歴など）は追跡しません（設計判断は「参考リソース」の Notion 引き継ぎ §2.1 参照）。進行中タイマーの永続化テーブルは持たず、**1 日 1 回の cron + `guild.members.fetch()` + フィルタ**で完結します（ステートレス。中規模公開 Bot 規模まで十分）。
...
- **事前警告（`warnDays`）**: キック予定の数日前に予告する。本人へは **DM**（大半のチャンネルが見えない未承認ユーザーに確実に届けるため）、サーバーへは **通知チャンネル**へ「キック予告」Embed を投稿（スタッフが事前に把握できるよう）。DM とチャンネル予告は同じ `warnDays` の時点で発火する。
...
| 事前警告 | 参加から `warnDays` 日経過した時点（= キックの `graceDays - warnDays` 日前）に、本人へ DM・通知チャンネルへキック予告 Embed（`warnDays` 未設定なら無効） |
...
しきい値 `G` = `graceDays`、警告日 `W` = `warnDays`（null なら警告無効）。

- `ageDays >= G` かつ 認証ロール未取得 → **キック対象**
- `W != null` かつ `ageDays == W` かつ 認証ロール未取得 → **事前警告対象**（この回はキックしない。本人へ DM・通知チャンネルへキック予告を発火。日次 cron + 整数日比較により 1 日 1 回・1 度だけ発火）

> **事前警告の重複防止**: 永続化テーブルを持たず「`ageDays` がちょうど `W` の日」を拾うだけで、24 時間ウィンドウに自然に 1 回送信となる（Notion 引き継ぎ §1.8 / §2.3）。`warnStage` のような per-member 状態は持たない。
>
> **注意（警告ゲートは無い）**: 本機能は §8 と異なり「警告済みでなければキックしない」というゲートを持たない。`warnDays` の事前警告とキックは独立しており、Bot が警告日（`ageDays == W` の日）にダウンしていた場合、そのメンバーは事前警告（DM・チャンネル予告）を受け取らずに `ageDays >= G` でキックされうる。これは Notion 引き継ぎ §1.8「DM が届かないユーザーには救済手段なし、これは仕様として割り切る」に沿った許容事項（→「制約・制限事項」）。`enabledAt` 起算下限により**有効化直後の無警告一斉キックは防がれる**が、ダウンタイム起因の単発取りこぼしは設計上許容する。
...
6. **事前警告**: 警告対象へ次を実行（`warnDays` が null なら本ステップ全体をスキップ）
   - **対象ロール付与**: `markerRoleId` 設定時、**通知投稿の直前**に警告対象（`ageDays == warnDays`）へ対象ロールを付与（付与済みなら何もしない）。これにより直後のメンションが解決される
...
`notifyChannelId` 設定時のみ、事前警告の発火時（`ageDays == warnDays`）に「キック予告」を**通知チャンネル**へ投稿する（スタッフ/メンバーが事前に把握するため）。DM が本人向け、これがサーバー向けで、両者は同時刻に発火する。member-log 流儀で**本文（content）はカスタム可変・Embed は固定構成**。
...
**Embed:** 区分別にメンバーと経過日数を表示。対象 0 件なら「対象なし」。区分は日次チェックと**同一定義**:

- キック対象 = `ageDays >= G` かつ 認証ロール未取得
- 事前警告対象 = `warnDays != null` かつ `ageDays == W` かつ 認証ロール未取得

preview は read-only で設定・DM・通知・キックを一切実行しない。
...
- 設定すると、日次チェックが警告対象（`ageDays == warnDays`）の確定後・通知投稿の直前に対象ロールを付与し、メンバーが**認証ロールを取得した時点**（`guildMemberUpdate`）で剥奪する。通知本文で `<@&markerRoleId>` としてメンションされる。
...
> 進行中状態（警告済みフラグ等）の per-member テーブルは持たない（ステートレス・Notion 引き継ぎ §1.4 / §2.3）。`createdAt` / `updatedAt` は saika の他 settings モデルに合わせて持たない。
...
- 警告 DM は per-member 状態を持たず「`ageDays == warnDays` の日」を拾うのみのため、**Bot がその日にダウンしていると警告 DM は送られないまま `graceDays` でキックされうる**（救済手段なし・Notion 引き継ぎ §1.8 の許容事項）。`enabledAt` 起算下限により有効化直後の無警告一斉キックは防がれる
- DM を受信拒否しているユーザーには警告が届かない（握りつぶす・仕様）
- 日次チェックは 1 日 1 回のため、警告 DM のタイミング・残日数は日単位（`ageDays == warnDays` でちょうど 1 回）
...
- [x] 段階判定（キック対象 = `ageDays >= G` / 警告対象 = `ageDays == W` / `warnDays` null で警告なし）
