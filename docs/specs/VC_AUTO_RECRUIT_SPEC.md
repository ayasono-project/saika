...
投稿対象は**明示的に有効化したカテゴリ（allowlist）に所属する VC のみ**です。プライベート/スタッフ用などのカテゴリに募集が漏れるのを防ぐため、管理者が「ここで募集して良い」カテゴリを `enable-category` で列挙し、**列挙されていないカテゴリの VC では投稿しません**（有効カテゴリ未設定の場合はどこにも投稿しません）。カテゴリに属さないルート直下の VC は **「TOP」** として同様に opt-in 対象です（`enable-category` をカテゴリ省略で実行すると TOP を有効化）。`@everyone` の閲覧可否では判定しません（認証制サーバーのメンバー専用 VC を誤って除外しないため）。
...
| enable-category | 募集を投稿する対象カテゴリ（allowlist）を追加（カテゴリ省略で TOP＝ルート） |
| disable-category | 対象カテゴリを解除（カテゴリ省略で TOP） |
| view | 現在の設定を表示 |
...
- 有効化はできるが、**有効カテゴリ（`enabledCategoryIds`）が未設定の場合は「カテゴリを有効化するまでどこにも投稿されない」旨を警告として併記する**（`enable-category` への導線）
...
## /vc-auto-recruit-settings enable-category

### コマンド定義

**コマンド**: `/vc-auto-recruit-settings enable-category`

**コマンドオプション:**

| オプション名 | 型 | 必須 | 説明 |
| --- | --- | --- | --- |
| `category` | Channel（カテゴリ） | ❌ | 募集を有効化するカテゴリ。**省略時は TOP（カテゴリなし＝ルート直下 VC）** |

### 動作フロー

1. `category` 指定時はそのカテゴリ ID、省略時は sentinel `"TOP"` を対象キーにする
2. `enabledCategoryIds` に対象キーを追加（重複時は「既に有効」を返す）
3. 完了メッセージを返信（ephemeral）

**ビジネスルール:**

- 既に有効化済みのカテゴリ/TOP を再追加した場合は「既に有効です」を返す（冪等）
- カテゴリ種別（`GuildCategory`）以外のチャンネルが渡された場合はエラー（Discord 側でカテゴリのみ選択可能だが実装でも検証）

### UI

**レスポンス（成功）:** `createSuccessEmbed` で対象カテゴリ（または TOP）の有効化を通知

---

## /vc-auto-recruit-settings disable-category

### コマンド定義

**コマンド**: `/vc-auto-recruit-settings disable-category`

**コマンドオプション:**

| オプション名 | 型 | 必須 | 説明 |
| --- | --- | --- | --- |
| `category` | Channel（カテゴリ） | ❌ | 募集を無効化するカテゴリ。**省略時は TOP** |

### 動作フロー

1. `category` 指定時はそのカテゴリ ID、省略時は `"TOP"` を対象キーにする
2. `enabledCategoryIds` から対象キーを除去（未登録時は「対象が見つかりません」を返す）
3. 完了メッセージを返信（ephemeral）

**ビジネスルール:**

- 未登録のカテゴリ/TOP を解除しようとした場合は「対象が見つかりません」を返す（冪等）

### UI

**レスポンス（成功）:** `createSuccessEmbed` で対象カテゴリ（または TOP）の無効化を通知
...
| フィールド: 有効カテゴリ | `<#categoryId>` の一覧（TOP は「TOP（カテゴリなし）」表示）/ 未設定（どこにも投稿されません） |
...
| `vc-auto-recruit-settings.enable-category.description` | サブコマンド説明 | 対象カテゴリを有効化（省略で TOP） | Enable a target category (omit for TOP) |
| `vc-auto-recruit-settings.enable-category.category.description` | オプション説明 | 有効化するカテゴリ（省略時は TOP＝ルート） | Category to enable (defaults to TOP/root) |
| `vc-auto-recruit-settings.disable-category.description` | サブコマンド説明 | 対象カテゴリを無効化（省略で TOP） | Disable a target category (omit for TOP) |
| `vc-auto-recruit-settings.disable-category.category.description` | オプション説明 | 無効化するカテゴリ（省略時は TOP） | Category to disable (defaults to TOP) |
...
| `user-response.category_enabled` | カテゴリ有効化成功 | {{category}} で募集を有効にしました。 | Recruitment enabled for {{category}} |
| `user-response.category_disabled` | カテゴリ無効化成功 | {{category}} で募集を無効にしました。 | Recruitment disabled for {{category}} |
| `user-response.category_already_enabled` | カテゴリ重複 | {{category}} は既に有効です。 | {{category}} is already enabled. |
| `user-response.category_not_found` | カテゴリ未登録 | {{category}} は有効カテゴリに登録されていません。 | {{category}} is not in the enabled categories. |
| `user-response.not_a_category` | カテゴリ種別エラー | カテゴリを指定してください。 | Please specify a category. |
| `user-response.enable_warning_no_category` | 有効化警告（カテゴリ未設定） | 有効化しましたが、有効カテゴリが未設定のため投稿されません。`/vc-auto-recruit-settings enable-category` でカテゴリを追加してください。 | Enabled, but no category is enabled so nothing will be posted. Add one with /vc-auto-recruit-settings enable-category. |
...
| `log.config_category_enabled` | カテゴリ有効化ログ | 有効カテゴリ追加 GuildId: {{guildId}} CategoryId: {{categoryId}} | enabled category added GuildId: {{guildId}} CategoryId: {{categoryId}} |
| `log.config_category_disabled` | カテゴリ無効化ログ | 有効カテゴリ削除 GuildId: {{guildId}} CategoryId: {{categoryId}} | enabled category removed GuildId: {{guildId}} CategoryId: {{categoryId}} |
...
- [ ] enable-category / disable-category: カテゴリ追加・解除・TOP（省略）・重複/未登録時の応答 / カテゴリ削除時の allowlist 掃除（channelDelete）
