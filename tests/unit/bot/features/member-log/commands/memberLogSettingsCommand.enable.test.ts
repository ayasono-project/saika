...
import { handleMemberLogSettingsEnable } from "@/bot/features/member-log/commands/memberLogSettingsCommand.enable";
...
vi.mock("@/bot/services/botCompositionRoot", () => ({
  getBotMemberLogSettingsService: () => ({
    getMemberLogSettings: (...args: unknown[]) =>
      getMemberLogSettingsMock(...args),
...
// handleMemberLogSettingsEnable の権限・前提条件チェック・成功フローを検証
describe("bot/features/member-log/commands/memberLogSettingsCommand.enable", () => {
  ...
  it("ガードが ValidationError を投げた場合にそれが伝播することを確認", async () => {
    ...
    await expect(
      handleMemberLogSettingsEnable(makeInteraction() as never, "guild-1"),
  ...
  it("channelId が未設定の場合に warning embed が返され setEnabled が呼ばれないことを確認", async () => {
    ...
    await handleMemberLogSettingsEnable(interaction as never, "guild-1");
  ...
  it("成功時に logger.info が呼ばれることを確認", async () => {
    ...
    await handleMemberLogSettingsEnable(makeInteraction() as never, "guild-1");
