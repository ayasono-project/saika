...
} from "@/shared/features/reaction-role/reactionRolePanelSettingsService";
...
describe("shared/features/reaction-role/reactionRolePanelSettingsService", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("createReactionRolePanelSettingsService ファクトリ関数がインスタンスを生成すること", () => {
    const repository = createRepositoryMock();
    const service = createReactionRolePanelSettingsService(repository as never);

    expect(service).toBeInstanceOf(ReactionRolePanelSettingsService);
  });
});
