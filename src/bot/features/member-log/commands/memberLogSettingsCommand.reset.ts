...
import { createDefaultMemberLogSettings } from "../../../../shared/features/member-log/memberLogSettingsDefaults";
import {
  logPrefixed,
  tInteraction,
...
const CUSTOM_ID = {
  CONFIRM: "member-log-settings:reset-confirm",
  CANCEL: "member-log-settings:reset-cancel",
} as const;
 ...
 */
export async function handleMemberLogSettingsReset(
  ...
  collector.on("collect", async (i) => {
    if (i.customId === CUSTOM_ID.CONFIRM) {
      ...
      const service = getBotMemberLogSettingsService();
      const defaultConfig = createDefaultMemberLogSettings();
