...
- 加えて、ダッシュボード（API）経由のリアクションロールパネル削除では、DB 行削除後に Discord メッセージ削除へ失敗した場合（孤児化リスク）の警告送信先としても使用される（イベント駆動ではないが、管理者の手動対応が必要なため通知する）
...
| 21 | リアクションロール | `reactionRoleResource.ts:137`（`deletePanelMessage`） | ダッシュボード経由削除で DB 行削除後に Discord メッセージ削除へ失敗（孤児化の可能性・手動削除/権限確認が必要）。運営 webhook には別途 `logger.error` で通知（webhook は level:error のみ捕捉のため） |

**通知対象外（debug/軽微/意図的な無視）:**

| # | 機能 | 内容 | 理由 |
| --- | --- | --- | --- |
| 22-23 | Bumpリマインダー | 旧パネル削除失敗 | debug レベル、後続処理に影響なし |
| 24 | メッセージ固定 | 旧メッセージ削除失敗 | debug レベル、既に削除済みの場合 |
| 25-26 | VC募集 | teardown編集失敗 | タイムアウト後の `.catch(() => {})` |
| 27-29 | VAC | チャンネル移動/削除/fetch | 意図的な `.catch(() => null)` |
| 30 | 共通 | `executeWithLoggedError` | VAC内部で使用 |
