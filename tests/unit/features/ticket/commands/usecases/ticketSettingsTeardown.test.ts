// tests/unit/bot/features/ticket/commands/usecases/ticketSettingsTeardown.test.ts
...
vi.mock("@/features/ticket/handlers/ui/ticketTeardownSelectHandler", () => ({
...
describe("bot/features/ticket/commands/usecases/ticketSettingsTeardown", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("設定が存在しない場合はエラー応答を返す", async () => {
    const { handleTicketSettingsTeardown } = await import(
      "@/features/ticket/commands/usecases/ticketSettingsTeardown"
    );

    findAllByGuildMock.mockResolvedValue([]);
    const interaction = createInteractionMock();

    await handleTicketSettingsTeardown(interaction as never, "guild-1");
