// src/features/inactive-kick/commands/inactiveKickSettingsCommand.schedule.ts
// inactive-kick-settings の スケジュール系サブコマンド（set-timezone / set-run-hour / mention enable|disable）
 ...
 */
export async function handleInactiveKickSetTimezone(
  ...
  const select = new StringSelectMenuBuilder()
    .setCustomId(INACTIVE_KICK_SETTINGS_COMMAND.SET_TIMEZONE_SELECT_ID)
    .setPlaceholder(
      tInteraction(locale, "inactiveKick:ui.select.set_timezone_placeholder"),
    )
    .addOptions(
  ...
  collector.on("collect", async (i) => {
    ...
    if (!timezone) return;
    await getBotInactiveKickSettingsService().setTimezone(guildId, timezone);
    await i.update({
      embeds: [
        createSuccessEmbed(
          tInteraction(
            locale,
            "inactiveKick:user-response.set_timezone_success",
            { timezone },
          ),
 ...
 */
export async function handleInactiveKickSetRunHour(
  interaction: ChatInputCommandInteraction,
  guildId: string,
): Promise<void> {
  await ensureInactiveKickManageGuildPermission(interaction);
  ...
  const select = new StringSelectMenuBuilder()
    .setCustomId(INACTIVE_KICK_SETTINGS_COMMAND.SET_RUN_HOUR_SELECT_ID)
    .setPlaceholder(
      tInteraction(locale, "inactiveKick:ui.select.set_run_hour_placeholder"),
    )
    .addOptions(
  ...
  collector.on("collect", async (i) => {
    ...
    await getBotInactiveKickSettingsService().setRunHour(guildId, runHour);
    await i.update({
      embeds: [
        createSuccessEmbed(
          tInteraction(
            locale,
            "inactiveKick:user-response.set_run_hour_success",
 ...
 * 個別メンション通知を有効化する。
 */
export async function handleInactiveKickMentionEnable(
  interaction: ChatInputCommandInteraction,
  guildId: string,
): Promise<void> {
  await ensureInactiveKickManageGuildPermission(interaction);
  ...
  await getBotInactiveKickSettingsService().setMentionEnabled(guildId, true);
  await interaction.reply({
    embeds: [
      createSuccessEmbed(
        tInteraction(locale, "inactiveKick:user-response.mention_enabled"),
        { title: tInteraction(locale, "common:embed.title.success") },
      ),
    ],
    flags: MessageFlags.Ephemeral,
 ...
 * 個別メンション通知を無効化する。
 */
export async function handleInactiveKickMentionDisable(
  interaction: ChatInputCommandInteraction,
  guildId: string,
): Promise<void> {
  await ensureInactiveKickManageGuildPermission(interaction);
  ...
  await getBotInactiveKickSettingsService().setMentionEnabled(guildId, false);
  await interaction.reply({
    embeds: [
      createSuccessEmbed(
        tInteraction(locale, "inactiveKick:user-response.mention_disabled"),
        { title: tInteraction(locale, "common:embed.title.success") },
      ),
    ],
    flags: MessageFlags.Ephemeral,
