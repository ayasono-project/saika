// tests/unit/bot/commands/afk-settings.test.ts
...
import { afkSettingsCommand } from "@/bot/commands/afk-settings";
import { handleCommandError } from "@/bot/errors/interactionErrorHandler";

type AfkSettingsInteraction = {
  guildId: string | null;
  channelId: string;
  locale: string;
...
// afk-settings 検証用の最小 interaction モック
function createInteraction(
  overrides?: Partial<AfkSettingsInteraction>,
): AfkSettingsInteraction {
  return {
    guildId: "guild-1",
    channelId: "channel-1",
    locale: "ja",
...
describe("bot/commands/afk-settings", () => {
  // ケースごとにモック状態を初期化する
  beforeEach(() => {
    vi.clearAllMocks();
    // tInteraction mock returns key as-is (no setup needed)
    getAfkSettingsMock.mockResolvedValue({ enabled: false, channelId: null });
  });
