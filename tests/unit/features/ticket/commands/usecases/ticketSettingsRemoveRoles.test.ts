// tests/unit/bot/features/ticket/commands/usecases/ticketSettingsRemoveRoles.test.ts
...
describe("bot/features/ticket/commands/usecases/ticketSettingsRemoveRoles", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("設定が見つからない場合はエラー応答を返す", async () => {
    const { handleTicketSettingsRemoveRoles } = await import(
    ...
    await handleTicketSettingsRemoveRoles(interaction as never, "guild-1");
