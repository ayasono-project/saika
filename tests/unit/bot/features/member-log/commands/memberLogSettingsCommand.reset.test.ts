...
import { handleMemberLogSettingsReset } from "@/bot/features/member-log/commands/memberLogSettingsCommand.reset";
...
vi.mock("@/shared/features/member-log/memberLogSettingsDefaults", () => ({
  createDefaultMemberLogSettings: () => ({ enabled: false }),
}));
...
describe("memberLogSettingsCommand.reset", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("確認ダイアログを ephemeral で表示する", async () => {
    ...
    await handleMemberLogSettingsReset(interaction as never, "guild-1");
  ...
  it("confirm ボタンで全設定をリセットする", async () => {
    ...
    await collectHandler({
      customId: "member-log-settings:reset-confirm",
      update: updateMock,
    });
  ...
  it("cancel ボタンでキャンセルメッセージを表示する", async () => {
    ...
    const updateMock = vi.fn().mockResolvedValue(undefined);
    await collectHandler({
      customId: "member-log-settings:reset-cancel",
      update: updateMock,
    });
